...
21. `Doc/TechnicalDocument/2026-06-27-phase-6-factorization-plugin-discussion-notes.md`：Phase 6 factorization 插件第一版拆分算法和主 TDD 对齐讨论记录；用于追溯“插件主导候选因子搜索空间分区”、同一个整数分解插件、canonical output 驱动递归展开的已确认决策，不覆盖字段规格。
...
当前 package layout 已确定为 `src/` layout；其中 `factorization/`、`lean_stub/`、`structured_report_stub/` 是 Phase 6 目标插件目录，`lean_stub/` 是历史占位名；真实 Lean 插件实现时可新增 `lean_proof/` 或保留 `lean_stub/` 作为兼容壳，但 descriptor / docs / tests 必须使用真实 Lean proof 语义。`experiments/` 是后续独立实验基础设施目录，当前实现阶段可按需创建：
...
| `tokenshare.plugins` | 插件契约、descriptor、factorization PoC、真实 Lean proof 插件、structured report stub PoC。 | 协议状态机推进、租约调度、正式输出绑定；Lean 规则不得进入 `tokenshare.core`。 |
