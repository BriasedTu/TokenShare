...
| `MockAIExecutor` | `src/tokenshare/executors/mock_ai.py` | 本地 deterministic fixture executor，不调用生产级 AI API 平台；保存 `RawModelOutput`、`ParsedModelOutput` 或 `ParseFailureReport` artifact，并返回统一 `ExecutionSubmission`。 | `tests/executors/test_mock_ai_executor.py` |
