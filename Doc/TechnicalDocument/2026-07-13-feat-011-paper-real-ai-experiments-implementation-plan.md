...
| D06 | Lean split 机制还窄，当前不能拆任意困难定理。 | 系统内部，条件触发 | 如果 hard catalog 需要 implication/forall/nested 支持，补 `lean_proof` 插件和 Lean helper。 |
...
- [x] 写 Lean catalog 30 题：easy/medium/hard 各 10 题，带 theorem payload、expected split kind、expected child count、minimum proof steps、context size、oracle proof/preflight ref。
...
- 如果 hard Lean 题仍使用 `P ∧ Q` / `P ↔ Q` 顶层，但 child proof 更长、更有干扰 context，这一步只需要实验层 catalog。
- 如果 hard Lean 题需要 `P → Q`、`∀`、nested conjunction/iff 等新顶层拆分，这一步必须输出 blocked/internal-gap，进入 Task 6B。
...
- [ ] 从 Lean catalog case 生成 theorem payload。
- [ ] 调用 Lean split helper，拿到 split certificate 或 structured unsupported。
- [ ] 对 child/direct proof 调真实 AI API。
- [ ] AI proof candidate 经过 Lean parser。
- [ ] proof candidate 经过本地 Lean checker。
- [ ] child proofs merge 后做 root recheck。
- [ ] per-task/per-attempt 记录 checker report、proof artifact、environment digest。
...
**层级：** 系统内部，仅当 Task 2 / Task 5 证明 hard catalog 被当前 split 机制挡住时执行。
...
- [ ] 故障选择必须 deterministic seed。
- [ ] fault 必须发生在真实 raw output 持久化之后。
- [ ] `false_positive` 修改 parsed candidate/submission。
- [ ] `false_negative` 抑制本来存在的 found candidate 或 proof。
- [ ] `no_return` 保存 raw 后不提交，让 lease expire。
- [ ] `late_submission` 超过 lease deadline 后提交，验证 late result 不污染 canonical。
- [ ] `executor_error` 保存 raw 后生成受控 error record，恢复 attempt 再真实调用 API。
- [ ] 每条 fault record 同时引用 original ref 和 mutated ref。
...
- [ ] 每个 worker 是独立 process 或等价的独立执行槽，有独立 worker id。
- [ ] 在 25% / 50% / 75% progress kill point 终止 worker。
- [ ] coordinator 不死，等待 lease expiry。
- [ ] replacement worker 接手并产生新的 attempt。
- [ ] 记录 worker pid、kill point、lease expiry、reassignment、replacement attempt。
...
- [ ] 支持 FULL、NO_PARSER、NO_VERIFICATION、NO_REQUEUE、NO_MERGE_GATE、NO_SLOT_INTEGRITY 等模式。
- [ ] 每个 ablation run 仍先真实调用 AI API。
- [ ] 关闭机制只发生在实验边界，不改变系统默认安全行为。
- [ ] 输出 canonical pollution、premature merge、blocked root、settlement blocked 等 evidence-derived 指标。
...
- [ ] fault 先选 1-2 类低比例。
- [ ] Lean hard catalog 如果大量 unsupported，回到 Task 6B 或调整 catalog 到当前明确支持的结构内。
...
- [ ] 每个 domain/difficulty 固定 5-task batch。
- [ ] repeats 5。
...
**这些实验现在能不能跑？**
不能直接跑成论文结果。当前 paper schema、catalog、budget/plan-only skeleton、real-AI eligibility gate 和 Lean catalog checker-backed preflight 已有基础；仍缺 factorization / Lean paper adapters、真实 API pilot 执行路径、post-AI fault / worker-death harness、evidence-derived metrics/report、secret scan wiring 和正式 Experiment 1-5 run。旧实验能跑，但只能算 regression/calibration。

**要跑还需要哪些东西？**
优先需要实验层代码，不是优先改系统核心：adapters、真实执行 runner、faults、workers、metrics、report、CLI 和正式运行编排。系统内部只在 pilot 证明挡路时补。

**准备的问题是不是太简单？**
旧 Lean 50 确实偏简单，因为主要是 `P ∧ Q` 和 `P ↔ Q`，child proof 常接近 `exact hP/hQ`。新 catalog 要用 difficulty、minimum proof steps、context 干扰和 checker preflight 来控制区分度。

**Lean 拆分机制是不是还没做好？**
当前不是“完全没做好”，而是“范围窄”。它可以拆已支持的顶层结构；不能拆任意困难 theorem。困难如果体现在 proof steps 变多，可以先不改 split；困难如果体现在新逻辑结构，就要补 Lean 插件内部 split/merge。
