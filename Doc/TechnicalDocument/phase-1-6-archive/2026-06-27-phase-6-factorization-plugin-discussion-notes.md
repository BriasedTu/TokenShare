...
2026-07-12 覆盖说明：最新实验设计以 `Doc/TechnicalDocument/tokenshare_latest_real_plugin_experiment_design.md` 为唯一权威。新论文 factorization 主实验必须让协议内部 range children 全部经过真实 AI API，再走插件 parser/verifier/canonical/merge；direct factorization 500 只能用于输入/oracle/成本校准。本文下方关于“早完成和剪枝是主 TDD 原始目标”的内容只保留为历史讨论来源，不再作为后续 paper runner 的验收口径。
