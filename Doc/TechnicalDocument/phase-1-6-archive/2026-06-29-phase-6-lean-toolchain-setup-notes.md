...
`feat-007` 已在后续实现中完成并标记 done；2026-07-12 起当前 active feature 是 `feat-011` Paper Real AI Experiments，`feat-010` Phase 9 replay / audit 已从当前剩余必做路径中延后。若未来扩展更多 Lean decomposition 规则，必须同时提供对应 verified merge policy 和 checker-backed 回归测试，不能只输出 supported split certificate。
