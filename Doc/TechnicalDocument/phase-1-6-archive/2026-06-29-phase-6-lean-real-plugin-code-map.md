...
2026-07-23 feat-011 runtime migration Task 9 addendum：`tests/integration/test_paper_protocol_runtime_integration.py` 使用 shared dispatcher + capturing transport + `RecordingLeanChecker` 验收一个 medium fixed lemma-DAG；certificate 仍由 Lean plugin 校验生成，所有 node 经过 `CHILD_PROOF`，最终 root 经过 `MERGE_PROOF`，engine ledger 覆盖 expand/dependency/canonical/merge/completion/settlement。为保持这条测试与正式调用链相同，`paper_dispatcher.dispatch_paper_case()` 增加只对 Lean 生效的可选 checker 透传；未提供时继续使用真实 checker。收尾另修复纯 catalog verify 的 manifest digest 时间漂移；零 recheck 时复用既有 manifest，真实 checker/source/entry 变化仍生成新证据。audit/CLI=`23 passed`，Task 9 修复后最终精确 Verify=`191 passed in 143.75s`，固定 real Lean canary=`11 passed in 101.74s`，最终增量 audit 600 reused/0 rechecked/provider calls=0，最终 Fast=`330 passed, 1 skipped in 52.17s`。

2026-07-23 feat-011 runtime migration Task 8 addendum：Lean paper adapter 现在把 `ProtocolRunRequest` 交给 shared dispatcher/coordinator，并由 `paper_projection.py` 从 system ledger/artifacts 派生公共 paper attempt/task shape；child checker accepted、canonical proof、dependency activation、merge/root `MERGE_PROOF`、completion 和 settlement 仍全部来自 Lean plugin runtime + engine。`RecordingLeanChecker` spy 明确验证每个 child 调用 `CHILD_PROOF` 且最终调用 `MERGE_PROOF`，因此 projection 不能替代 checker。Lean adapter 全文件 `26 passed`，固定 canary `11 passed`；没有运行 600-entry audit 或 provider/formal paper run。

状态：`feat-007` 完整实现与审查硬化映射。本文记录真实 Lean proof plugin 的 source、Lean fixture project、测试、协议边界和验证证据。2026-06-29 审查硬化已关闭 `sorry` 接受、proof candidate schema 绑定、split policy / elaboration gate、unsupported intro merge、child proof artifact merge、真实 preflight 和 environment digest 稳定性问题。2026-07-22 feat-011 runtime migration Task 5 进一步把预注册 fixed plan 校验/certificate 和 Lean FULL lifecycle bridge 收入插件，并由 system runtime/`ProtocolEngine` 记录权威生命周期；这不是通用自动 lemma discovery。Phase 6 的 structured report stub 已从开发计划剔除；历史名称只作为 provenance / 通用夹具，不是剩余目标。
...
| `src/tokenshare/plugins/lean_proof/fixed_plan.py` | 2026-07-22 feat-011 runtime migration Task 5 | 定义 versioned `LeanFixedDecompositionPlan`，把 catalog 的预注册 nodes/edges/merge shape 收进插件 API；校验 DAG、唯一 root、node/edge/slot 完整性、depth/leaf/count、environment/oracle digest，并统一生成既有 v2 lemma-DAG certificate。 |
| `src/tokenshare/plugins/lean_proof/runtime_adapter.py` | 2026-07-22 feat-011 runtime migration Task 5 | Lean runtime bridge：规划 simple/v2 proof units、构造 proof-candidate request、调用 checker、把 checker accepted proof artifact 提升为 canonical output，并复用 plugin merge policy 做 dependency-aware assembly/root recheck；协议状态只由 coordinator/engine 推进。 |
...
| `tests/plugins/lean_proof/test_lean_fixed_plan.py` | 2026-07-22 Task 5 fixed-plan schema/validation RED-GREEN：覆盖合法 catalog、DAG/root/node/edge/slot、depth/leaf/count、environment/oracle digest 与 structured-blocked 边界。 |
| `tests/plugins/lean_proof/test_lean_runtime_adapter.py` | 2026-07-22 Task 5 runtime bridge：覆盖 v2 plan unit/capability/dependency-edge 映射，确保 proof units 要求 AI executor、拆分权威仍归插件。 |
...
| `tests/local_runtime/test_lean_fixed_plan_runtime.py` | 2026-07-22 Task 5 system lifecycle：simple 与 fixed lemma-DAG 都走 coordinator；验证 scheduler/lease/request/submission、checker-before-canonical、dependency input refs、engine 依赖解阻事件、merge/root recheck、completion/settlement。 |
