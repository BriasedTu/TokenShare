...
`Doc/TechnicalDocument/tokenshare_latest_real_plugin_experiment_design.md` 执行。
...
论文实验和 paper runner / metrics 设计只以
`Doc/TechnicalDocument/tokenshare_latest_real_plugin_experiment_design.md` 为权威：

- Experiment 1 使用真实 AI API，在 factorization 和 Lean 三档难度输入上验证跨领域可行性、完成率、accepted validity、时间、token 和失败边界。
- Experiment 2 固定任务和模型，用 1/3/10/30 workers 测端到端时间、吞吐、speedup、efficiency、token、成本和 provider 限流；100/300 只在任务粒度和 quota 支持时扩展。
- Experiment 3 在真实 API 输出后注入 false positive、false negative、不返回、延迟、executor error 和独立 worker process death，测检测率、恢复率、完成率和成本曲线。
- Experiment 4 所有 run 仍调用真实 AI API，每次只关闭 verification、parser policy、requeue、merge gate 或 slot integrity 中的一个机制。
- Experiment 5 比较 strong-only、weak-only 和 difficulty-aware mixed，作为次要模型分配分析。

旧 Phase 8 Experiment 1-4、scripted/deterministic suite、direct factorization 500 和浅层 Lean AI 50 只用于 regression/calibration，不能作为新论文主结果。
