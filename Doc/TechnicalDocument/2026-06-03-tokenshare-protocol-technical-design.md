...
可扩展的协议框架。整数分解、真实 Lean 形式化证明和结构化报告是三类实验插件，用于验证
协议框架能否承载不同类型的任务。
...
和真实 protocol lifecycle 展开；旧 toy range demo、raw-only demo 或 `lean_stub` 结果不能作为实验通过证据。
...
- 提供 factorization、真实 Lean 形式化证明插件和 structured report stub 三类插件作为实验对象。
...
让同一 structured report 流程使用真实模型输出验证效果。V1 不把该能力扩大为生产
...
| `tokenshare.plugins` | 任务插件 | 插件契约、descriptor、factorization PoC、真实 Lean proof 插件和 structured report stub PoC。 |
...
- 三类插件实验均能跑通：强验证计算、真实 Lean proof checking、自然语言结构化报告。
...
| AI API SDK 或 HTTP client | 实验依赖 | 真实模型输出效果验证。 | Phase 7 已引入实验级 executor；API key 只来自环境变量，baseline 测试不得要求联网，replay 不得重新调用 API。 |
...
- 真实 Lean 形式化证明和 structured report stub 三类插件实验均能按各自插件边界端到端完成。
- 事件日志可重放最终状态。
- 正式输出唯一。
- sandbox 结算恰好一次。
- 故障模拟结果符合预期。
