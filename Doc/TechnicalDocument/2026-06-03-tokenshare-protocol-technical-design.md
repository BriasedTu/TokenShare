...
2026-06-23 起，本文吸收 P01-P22 候选机制规范中的 V1 取舍，成为后续 Phase 3 到 Phase 7
...
1. **任务图与输出解析分离**：`TaskRelation` 继续表达拆分树和执行依赖 DAG；
   ...
   合并任务满足。端口 schema 是类型兼容的权威来源，edge 上的 schema snapshot 只作审计。
2. **动态扩图只消费正式事实**：V1 不做 speculative 子图。只有来源 attempt 已验证并
   成为 canonical 后，`DecompositionProposal` 才能被接受；child task、edge 和 expected
   output ID 从已保存 proposal artifact 的 canonical digest 与逻辑位置确定性派生。
...
4. **契约先分硬约束和偏好**：插件 descriptor 声明 typed ports、hard requirements、
   soft hints、validator policy 和 merge policy；调度时保存 capability snapshot/digest 和
   deterministic `AllocationDecision`，但 V1 不实现完整 Contract Net bidding。
...
| `MetricsCollector` | 协议框架 | 从事件日志和实验配置生成 completion、work/span、retry/wasted work、shadow benefit 和 settlement 指标。 |
...
| `ExpectedOutputRef` | 描述父节点需要的命名输出如何被直接 attempt、子图或 merge task 满足。 | 输出名、期望类型、owner task、resolution 状态、来源 canonical bundle 或 merge plan。 |
...
10. 协议按 `first_verified_bundle` 绑定正式输出束，并把 `CANONICAL_OUTPUTS_BOUND`
    作为逻辑 `CanonicalSelection` 记录。
...
当前分层 package layout 与 Phase 6 目标插件目录：
...
- `expand` 必须先记录结构化 proposal 和 expansion decision，再用已保存 proposal 的 canonical digest 派生子节点、expected outputs 和依赖 ID。
- 无效展开不会写入任务图。
...
| `ExpectedOutputRef` | 父任务所需命名输出的最小 future/resolution 引用。 |
