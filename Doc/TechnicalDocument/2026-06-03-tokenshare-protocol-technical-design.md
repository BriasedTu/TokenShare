...
可扩展的协议框架。当前实验插件目标为整数分解和真实 Lean 形式化证明，用于验证
协议框架能否承载不同类型的任务。结构化报告 stub 曾作为第三个实验插件计划存在，但已在 2026-06-29 从 Phase 6 开发计划中剔除；历史夹具名称只保留为 provenance。
...
2026-06-28 起，Lean 相关范围按 `2026-06-28-phase-6-lean-real-plugin-scope-change.md` 覆盖旧口径：
Phase 6 第二插件不再是 stub / synthetic-only proof，而是本地真实 Lean checker 驱动的形式化证明插件。
V1 不做生产级 theorem-proving 平台或 LeanDojo 训练/检索平台，但必须实现可复现、可审计的真实 Lean proof checking path。

2026-06-29 起，论文实验和实验基础设施的主口径按
`Doc/TechnicalDocument/tokenshare_latest_real_plugin_experiment_design.tex` / `.pdf` 执行。
该设计要求实验围绕真实插件边界、真实 descriptor、真实 artifact、真实 parser / verifier / merge policy
和真实 protocol lifecycle 展开；旧 toy range demo、raw-only demo 或 `lean_stub` 结果不能作为实验通过证据。structured report stub 已从 Phase 6 开发计划中剔除，不再作为 V1 待实现插件或完成标准。
...
- 提供实验级 AI API executor，使 proof-like / natural-language fixture 等受控任务可以在真实模型输出上做效果验证；该 executor 仍走本地协议 envelope、artifact 持久化和固定环境身份，不提供生产 AI 平台能力。
...
- 提供 factorization 和真实 Lean 形式化证明插件作为实验对象；structured report stub 已从 Phase 6 开发计划剔除。
...
- 生产级 theorem-proving 平台、LeanDojo 训练/检索平台或动态 Lean 服务；Phase 6 的本地真实 Lean checker 插件属于 V1 范围内。
...
- 信誉向量、质押、惩罚和真实结算后端。
- 人类 worker 和远程服务执行器。
- 更完整的 LeanDojo / premise retrieval / miniF2F 大规模 benchmark 实验。

## 4. 设计目标与非目标

### 4.1 设计目标

- **协议优先**：协议核心不理解 factorization、Lean 或历史 structured report fixture 的领域知识。
...
目标：验证协议的递归拆分、调度、验证、all-required merge、失败恢复、结算和 replay。

V1 设计：

- 插件身份为 `factorization@0.1.0`，主要策略包括
  `factorization.candidate_range_partition.v1`、
  `factorization.range_result.parser.v1`、
  `factorization.range_result.validator.v1` 和
  `factorization.all_required_range_merge.v1`。
- 根输入包含待分解整数和 requested output；root input 不是最终结果，必须先生成并
  canonical 绑定 `factor_integer_subject`。
- 拆分策略只消费 canonical subject，并把候选因子搜索空间分割为无 gap / overlap 的
  `factor_search_range` 子任务；coverage proof 必须检查 full domain 和 sqrt bound。
- 执行器只能搜索分配到的 bounded range，不能搜索区间外、创建子任务、声称最终分解结果、
  决定奖励或修改任务图。
- AI / mock-AI 路径必须使用插件生成的 `PromptPackage`，raw output 必须经插件 parser
  转成 `factorization.range_result.v1`，raw-only / free-form 输出不能进入成功 verification 或 canonical。
- 验证器检查 `found_factor` 的 range、divisibility、cofactor 和 coverage/params；
  `no_factor_in_range` 必须 deterministic bounded recheck。
- 第一切片采用 all-required merge：所有 required range slots canonical 后才创建 merge task。
  不实现 `one_success`、提前完成、sibling pruning 或 composite cofactor 的完整递归 resolution。
- prime fixture 期望所有 range 输出 `no_factor_in_range` 后生成 `prime_certificate` 和
  `PrimeFactorizationResult([n])`；semiprime fixture 只有在 factor 与 cofactor 均有 primality evidence
  时才能解析为 `PrimeFactorizationResult([p,q])`。
- composite cofactor case 只能输出 `nontrivial_factor_found` 和 limitation reason，不能伪装为 final result。

### 14.2 Lean 形式化证明插件

目标：验证协议能承载真实形式化证明任务、固定 Lean 验证环境、proof artifact 输出、
checker diagnostics、递归子目标和子证明合并。

V1 设计：

- 根输入包含 theorem 标识、Lean theorem / proof-state 代码、imports、namespace/context 和固定环境摘要。
- `EnvironmentRef` 固定 Lean executable、lake project、toolchain/library/import set、namespace/context、配置 digest 和展示用摘要。
- 插件内 deterministic parser / normalizer / split strategy 根据 Lean 命题结构自动生成子目标；AI 或 executor 不得决定协议级拆分。
- AI API executor 或本地 executor 可生成 proof candidate，但候选必须作为 artifact 进入统一 `ExecutionSubmission`。
- 插件解析 proof artifact、Lean checker stdout/stderr、exit code、timeout 和 diagnostics。
- 验证器调用固定本地 Lean/lake 环境真实检查 proof artifact；验证结论引用 `EnvironmentRef`、proof input digest 和 checker log artifact。
- 子目标展开为普通 `TaskUnit`，且只由插件版本化 split strategy 生成 `DecompositionProposal` / `MergePlan`。
- 合并规则要求 required 子证明均有 canonical proof artifact 后，再生成父 proof assembly / final proof artifact 并通过 Lean checker。

Lean V1 不再是 one-shot synthetic fixture stub。V1 不实现 LeanDojo 训练、DPR/premise retrieval、best-first proof search 平台或生产级 Lean 服务；
但必须实现本地真实 Lean checker 驱动的 direct proof 路径和 decomposition / merge 路径。若使用 miniF2F 或其他 benchmark 子集，必须固定版本、problem IDs、split 和本地资料索引。

### 14.3 结构化报告 Stub 插件（已从 Phase 6 计划剔除）

2026-06-29 状态：本节保留为历史设计 provenance，不再作为 V1 / Phase 6 待开发插件目标，也不进入后续完成标准。若代码或测试仍使用 `structured_report_stub` 名称，应视为早期 generic plugin contract fixture，不得据此恢复插件开发范围。

目标：专门验证大型自然语言任务的“插件规则拆分、AI 文本验证、合并”路径，弥补整数分解和
Lean 形式化证明对普通文字任务覆盖不足的问题。
...
旧计划曾要求 Phase 6 中该插件使用本地 fixture / stub，先验证自然语言大型任务中最容易出错的拆分、
验证和合并边界。已完成的 Phase 7 实验级 AI API executor 可在不改变插件和协议边界的前提下，
该实现计划已废弃；Phase 7 实验级 AI API executor 仍可用于未来受控 natural-language fixture。V1 不把该能力扩大为生产
AI 平台；API 调用结果必须作为 artifact 持久化，replay 不得重新调用模型。

### 14.4 最新真实插件实验矩阵

论文实验和 Phase 8 runner / metrics 设计以
`Doc/TechnicalDocument/tokenshare_latest_real_plugin_experiment_design.tex` 为主：

- Experiment 1 验证 `factorization@0.1.0` 真实插件端到端生命周期：
  descriptor freeze、root subject canonical、split、range execution、parser/verifier、
  canonical binding、all-required merge、expected output resolution、settlement 和 ledger replay。
- Experiment 2 分别注入 invalid found factor、false no-factor、parse failure / raw-only、
  worker crash / expired lease 和 no-factor recheck budget exceeded，验证这些失败不会污染 canonical state。
- Experiment 3 做 protocol ablation，关闭 verification、parser policy、requeue、all-required merge gate
  或 slot integrity，验证关键机制缺失时出现预期退化。
- Experiment 4 用 factorization plugin 和真实 Lean proof plugin / Lean adapter 验证跨插件泛化。
  如果真实 Lean checker logs、proof artifact 和 `EnvironmentRef` 不存在，该实验只能标记为
  blocked / pending real Lean plugin，不能用 `lean_stub` 或 synthetic-only 结果替代。
...
- factorization Experiment 1 能完成 prime / semiprime fixture，并产生可 replay 的 descriptor、artifact、verification、canonical、merge 和 settlement 证据。
- Experiment 2 的 invalid factor、false no-factor、parse failure/raw-only、worker crash/expired lease 和 budget exceeded case 均不能污染 canonical state。
- Experiment 3 的消融模式能呈现预期退化，特别是 no verification、no parser policy、no requeue、no all-required merge gate 和 no slot integrity 的风险。
- Lean 形式化证明实验能在固定本地 Lean 环境中完成至少一条 direct proof 路径和一条 decomposition / 子证明合并路径。
- Experiment 4 必须使用真实 Lean proof plugin / Lean adapter；没有真实 checker logs 和 `EnvironmentRef` 时不得宣称跨插件泛化通过。
...
| Lean 环境集成过重 | 真实 checker 会增加 toolchain、lake project、library 版本和 CI 环境成本。 | 高 | 固定本地 Lean/lake/toolchain/library `EnvironmentRef`，targeted tests 可在缺少 Lean 时 fail fast / skip，并把 checker log artifact 化。 |
...
| Lean 实验 | 本地真实 Lean checker | 检查 Lean theorem/proof artifact、diagnostics、proof-state 拆分和子证明合并。 | 必须固定 Lean/lake/toolchain/library `EnvironmentRef`；不实现生产级 theorem-proving 平台。 |
...
src/
  tokenshare/
    ...
    plugins/
      factorization/
      lean_stub/  # 历史占位；真实 Lean 插件实现时可迁移为 lean_proof/ 或保留兼容壳
    executors/
    replay/
    experiments/
...
| `tokenshare.plugins` | 任务插件 | 插件契约、descriptor、factorization PoC 和真实 Lean proof 插件；历史 structured report fixture 只作为旧测试 / contract 示例保留。 |
| `tokenshare.executors` | 执行器 | 执行器契约、本地 mock executor、确定性执行器和实验级 AI API executor。 |
...
- Phase 6 接入真实 Lean 时，只能扩展 Lean 插件、executor 层和 artifact schema，
  不应改写协议核心状态机；人类 worker 仍属于后续迁移方向。
...
- 整数分解插件。
- 真实 Lean 形式化证明插件。
- 两类插件的 descriptor、typed ports、root/child schema、execution contract、output contract、validator、split strategy、merge/pruning policy 和本地 fixture。

验收：

- 两类插件实验均能跑通：强验证计算、真实 Lean proof checking。
- Lean 插件使用固定本地 Lean checker；拆分算法无 AI 介入；checker 环境、proof artifact、日志和 diagnostics 必须 artifact 化。
...
- API key 和 endpoint 配置的本地读取规则；标准 executor config 只保存 `api_key_env`，真实 smoke 可从被 gitignore 的本地 JSON 注入当前进程环境变量，secret 不得写入 event、artifact、SQLite、日志或 config digest。
- `RawModelOutput`、`ParsedModelOutput`、`ParseFailureReport`、tool/action/observation provenance 与现有 `ExecutionSubmission` 的兼容路径。
- 针对受控插件 fixture 的最小真实模型 smoke flow；CI / baseline 不要求联网或真实 API key。
...
- 至少一个受控插件 fixture 可以选择 mock 或真实 AI API executor 运行，且不让 executor 修改任务图、绑定 canonical output、决定奖励或自行重试。
...
- Experiment 1-4 的 runner 入口或等价测试入口：factorization end-to-end、failure injection、protocol ablation、real-plugin generality。

验收：

- 五类故障模拟均通过。
- 指标报告可生成，并包含最新实验设计要求的 correctness、canonical pollution、parse failure isolation、requeue、premature merge、slot mismatch、shared lifecycle coverage、real checker evidence、work、critical path、retry/wasted work 和 shadow benefit。
- ExperimentRunner 可以运行 Experiment 1-3 的 factorization cases；Experiment 4 只有在真实 Lean plugin / Lean adapter 可用且保存 checker logs / `EnvironmentRef` 后才能标记通过。
- Phase 6 插件以及 Phase 7 AI API executor 路径可以被实验 runner 作为 fixture/benchmark case 运行，但实验基础设施不反向写入插件领域逻辑。
...
| Lean `EnvironmentRef` / lake project | 实验依赖 | 真实 Lean proof 验证。 | V1 必须固定本地 Lean/lake/toolchain/library/context；不做生产级 Lean 服务或 LeanDojo 训练平台。 |
| 结构化报告 fixture | 历史 / 可选实验依赖 | 自然语言 section 输出、证据引用、缺失 section 和伪造引用测试。 | 已从 Phase 6 开发计划剔除；保留为历史设计 provenance，不作为完成标准。 |
| AI API SDK 或 HTTP client | 实验依赖 | 真实模型输出效果验证。 | Phase 7 已引入实验级 executor；标准 config 只保存 `api_key_env`，真实 smoke 可读取被 gitignore 的本地 JSON 并只注入当前进程环境变量；baseline 测试不得要求联网，replay 不得重新调用 API。 |
...
| Lean 插件真实环境与拆分算法 | V1 要求覆盖 direct proof 与 decomposition / 子证明合并，并使用真实 Lean checker。 | 插件字段规格中固定 Lean 环境、proof artifact schema、parser/split strategy、checker 调用和测试前置条件。 |
| Structured report stub fixture | 已剔除 | 2026-06-29 起不作为 V1 / Phase 6 待实现插件目标。 |
...
- 最新真实插件实验设计中的 Experiment 1-3 能以 factorization 真实插件运行并产生可 replay 证据。
- Experiment 4 能以 factorization plugin 和真实 Lean proof plugin / Lean adapter 运行；若 Lean 插件或本地 checker 环境未完成，必须明确标记 blocked / pending，不能用 stub 结果替代。
- 真实 Lean 形式化证明插件能按插件边界端到端完成；structured report stub 不再属于完成标准。
- 事件日志可重放最终状态。
- 正式输出唯一。
- sandbox 结算恰好一次。
- 故障模拟结果符合预期。
