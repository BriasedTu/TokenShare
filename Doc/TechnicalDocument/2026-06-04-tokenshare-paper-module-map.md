...
| 任务单元建模：中间 reasoning/proof state | 只有具备稳定 schema、typed I/O、可独立调度、明确 validator 和受控图关系的 durable subgoal/proof state 才能晋升为 TaskUnit；普通 thought、vote、working memory 保持 attempt artifact 或执行器私有状态。 | P17: *Tree of Thoughts: Deliberate Problem Solving with Large Language Models* | `tokenshare-paper-tex/P17_tree_of_thoughts.tex` | <https://papers.neurips.cc/paper_files/paper/2023/file/271db9922b8d1f4dd7aaef84ed5ac703-Paper-Conference.pdf> |
...
| Byzantine 控制面复制边界 | `3f+1` replica、认证消息、view/checkpoint 与 prepare/commit quorum 只适用于满足独立 Byzantine fault 和确定性状态机等前提的未来控制面复制；不得直接作为 AI verifier committee 或语义答案验真阈值。V1 不实现 PBFT。 | P11: *Practical Byzantine Fault Tolerance* | `tokenshare-paper-tex/P11_pbft.tex` | 原草稿源：<https://www.usenix.org/conference/osdi-99/presentation/practical-byzantine-fault-tolerance>；下载源：<https://pdos.csail.mit.edu/6.824/papers/castro-practicalbft.pdf> |
...
| 语义任务：latent label 与 observer reliability | 仅在没有 gold label、输出可化为有限离散标签、存在交叠观察且条件独立等假设可接受时，用 EM 联合估计潜在标签和观察者错误率。它是未来 weak semantic verification 插件参考，不进入 factorization/Lean 强 checker 路径，也不能把自由文本一致性直接当正确性。 | P12: *Maximum Likelihood Estimation of Observer Error-Rates Using the EM Algorithm* | `tokenshare-paper-tex/P12_dawid_skene.tex` | 原草稿源：<https://doi.org/10.2307/2346806>；下载源：<https://crowdsourcing-class.org/readings/downloads/ml/EM.pdf> |
...
| AI 执行日志与工具 provenance | 强制保存 action、observation、tool/version、输入输出 digest 和错误；hidden reasoning trace 只作为可选 raw artifact，不是协议真值或 replay 前置条件。 | P18: *ReAct: Synergizing Reasoning and Acting in Language Models* | `tokenshare-paper-tex/P18_react.tex` | <https://arxiv.org/pdf/2210.03629> |
| Lean 证明插件：proof state、premise retrieval、verifier | Phase 6 Lean 插件改为真实 Lean checker 驱动：使用固定本地 Lean/lake/toolchain/library 环境检查 proof artifact；LeanDojo 的 proof-state/verifier 工作流用于约束 EnvironmentRef、checker log、diagnostics 和 replay 边界，但不要求 V1 实现 DPR 训练或完整 best-first search 平台。 | P19: *LeanDojo: Theorem Proving with Retrieval-Augmented Language Models* | `tokenshare-paper-tex/P19_leandojo.tex` | <https://proceedings.neurips.cc/paper_files/paper/2023/file/4441469427094f8873d0fecb0c4e1cee-Paper-Datasets_and_Benchmarks.pdf> |
| Lean 实验 benchmark | Phase 6 可使用真实 Lean theorem / proof-state fixture 或受控 miniF2F 子集；若采用外部 benchmark，必须固定版本、problem IDs、split 和本地材料索引，不能只引用在线链接。 | P20: *miniF2F: a Cross-System Benchmark for Formal Olympiad-Level Mathematics* | `tokenshare-paper-tex/P20_minif2f.tex` | <https://arxiv.org/pdf/2109.00110> |
| Lean verifier 环境背景 | mathlib 说明 proof checker 依赖具体 library/toolchain/namespace 语境；V1 的 `EnvironmentRef` 必须固定 Lean/lake/library/import/namespace/context，而不是把 checker 结论当成与环境无关的事实。 | P21: *The Lean Mathematical Library* | `tokenshare-paper-tex/P21_lean_mathlib.tex` | 原草稿源：<https://doi.org/10.1145/3372885.3373824>；下载源：<https://leanprover-community.github.io/papers/mathlib-paper.pdf> |
| 路由/认领/指派机制 | V1 只吸收 eligibility、assignment/no-match reason、deadline 与 report/refusal 语义，采用本地 deterministic direct assignment；完整 bid/award 协商市场留到后续版本。 | P22: *The Contract Net Protocol: High-Level Communication and Control in a Distributed Problem Solver* | `tokenshare-paper-tex/P22_contract_net_protocol.tex` | 原草稿源：<https://doi.org/10.1109/TC.1980.1675516>；下载源：<https://www.reidgsmith.com/The_Contract_Net_Protocol_Dec-1980.pdf> |
