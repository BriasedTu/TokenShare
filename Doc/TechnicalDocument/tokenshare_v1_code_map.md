...
| `artifacts.py` | content-addressed artifact、manifest、hash/URI 验证；Task 8 提供 temp flush/fsync/rename 与 commit marker 顺序，使外部 bank object 可在 crash 后确定性 reconcile。 |
...
2026-08-01 已冻结 Experiment 2–4 两阶段真实回答库设计。当前 35 个实施 Task 中 Task 0–8 已 accepted：Task 8 durable acquisition/publish/resume/reconcile commit=`6802918d`，严格六个实际变更文件；最终 exact 5-file command exit `0`、`77 passed in 15.20s`，manifest 专项 `3 passed`/cross-check `4 passed`，fake-only 且真实 provider/network/secret=`0`；4 个 production 文件 `py_compile`、diff-check、allowlist、secret scan exit `0`；final review PASS、Critical/Important/Minor=`0/0/0`，manifest 修复闭合；test minimization 3 分钟=`NO_CHANGE`，collect-only=`19`（行为 `18`）。这个 `9/35` 状态不代表整条 paper pipeline 已实现：Task 9 deterministic logical source-latency scheduler 为 queued/next，`paper_formal_runner.py` 尚不能以回答库输入重新驱动完整状态机。本次没有 paid receipt，未运行 Fast/Full/LeanAudit，未联网或调用 provider。

后续完整实施计划必须同时覆盖：

- deterministic logical source-latency scheduler（response-bank acquire/publish/resume/reconcile 与 actual usage 写入已由 Task 8 提供）；
