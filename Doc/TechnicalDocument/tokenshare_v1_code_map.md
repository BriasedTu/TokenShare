...
| `experiments/slim_v2/profiles.py` | `build_profile()` 与 `_provider_identity()` 冻结前向 Exp1 的 `deepseek_v4_flash_exp1_baseline` / `deepseek-v4-flash`，再由 `project_root_inventory_rows()` 写入每个 root 的显式 provider/model；Full Exp5 冻结为 28 个 Factorization hard + 三个 Lean topic 各3题、GLM/Qwen/MiniMax 三个thinking模型、仅 `repeat0`/`max_retries=0`（12 conditions、111 roots、852 planned first attempts），并仍只从其自身 endpoint 映射取得身份。 |
...
| `experiments/slim_v2/runtime.py` | 把scenario scheduler/policy/hooks/backend注入同一个`ProtocolRunCoordinator.run_root`；生产root装配与全实验strict-v2 protocol投影共用同一路径。Exp1 fresh/resume都从protocol snapshot attempts与当前root全部已持久化coverage-tail traces重建最终attempts，顺序为protocol原顺序再按冻结tail target/自然ordinal；已有trace和terminal call不重复。`_pre_flash_terminal_root_facts()` 与 `reproject_pre_flash_exp2_terminal_root_context()` 仅对一个冻结 Exp2 key，从既有 terminal ledger/artifacts/trace 重建 fixed-response typed facts 并写 projection，明确不重跑 protocol、provider 或 transport；重复 submission、非终态或任何非 exact cohort 均拒绝。有效`no_final`继续tail，tail资源只计真实provider calls，不建立第二runner或状态机。 |
