...
| `tests/experiments/slim_v2/test_reducer_golden.py` | 单一golden run风险驱动覆盖153/153、fixed denominator、tail、独立pair eligibility、六组四端、Exp5分类、bootstrap/null阈值与发布事务；另覆盖strict-v2读取、Exp2 stale reason清理、Exp3/4/5正式零分母reason、committed failure taxonomy，以及infra cell保留精确事实和pair reason传播。Exp5额外覆盖 parsed-but-unsubmitted 的可评估性边界、缺既有N/A标记时fail-closed、仅repeat0墙钟兼容，以及C root不污染可检查验证拒绝率的bootstrap；还覆盖补充表保留三个实测模型的wall-clock、而Exp1 Flash V4只保留质量/token/成本且wall-clock全为N/A。 |
