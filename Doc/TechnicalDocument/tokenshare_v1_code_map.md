...
Slim V2 当前入口与权威只在`Doc/SlimV2/`；运行代码位于`src/tokenshare/experiments/slim_v2/`，不得import旧`paper_*`/formal runner。最终汇总 Tasks 4–6 的既有改动仍归入`schema.py/storage.py/projector.py/runtime.py/cli.py/reducer.py`及其 focused tests；2026-08-23 的最小补丁精确覆盖 lease-expiry projector、唯一 pre-Flash terminal-ledger reproject、前向 Flash model/price mapping 及其 tests。2026-08-27 的 reducer 补丁只闭合 Exp1 usage 缺失 totals 与 Exp5 parsed-unsubmitted/infrastructure-invalid 组合 cell 的科学 missing reason，并使用现有 Full run 发布正式 metrics；表中其他既有职责与 dirty-tree shared 改动不归因于本轮。
...
| `experiments/slim_v2/profiles.py` | `build_profile()` 与 `_provider_identity()` 冻结前向 Exp1 的 `deepseek_v4_flash_exp1_baseline` / `deepseek-v4-flash`，再由 `project_root_inventory_rows()` 写入每个 root 的显式 provider/model；`downstream_trace_consumer_case_ids()`仅从同profile Exp2–4实际inventory派生Exp1 tail-required case闭包，`coverage_tail_required_by_downstream()`把它映射到单root（Full实证99来源/336非来源，非硬编码）。Full Exp5冻结为28个Factorization hard+三个Lean topic各3题、GLM/Qwen/MiniMax三个thinking模型、仅`repeat0`/`max_retries=0`（12 conditions、111 roots、852 planned first attempts），并仍只从其自身endpoint映射取得身份。 |
| `experiments/slim_v2/execution.py` | Exp1/provider与fixed-trace submission边界；`_provider_attempt()` 统一把 `provider.project_cost()` 的按模型/版本成本投影写入 attempt。Task 4增加parser前P route、Lean normalize/checker前Slim bridge以及Process结果透明prepare/export/ingest。 |
| `experiments/slim_v2/runtime.py` | 把scenario scheduler/policy/hooks/backend注入同一个`ProtocolRunCoordinator.run_root`；生产root装配与全实验strict-v2 protocol投影共用同一路径。CLI把同profile Exp2–4 trace-consumer closure派生的`coverage_tail_required_by_downstream`以必填bool固化入每个root context；Exp1 runtime在装配provider/root前直接读取并严格校验，遗漏或非法值fail-fast，绝不猜测为来源。来源Exp1 fresh/resume从protocol snapshot attempts与当前root已持久化coverage-tail traces重建最终attempts，顺序为protocol原顺序再按冻结tail target/自然ordinal；非来源Exp1以`not_required_by_downstream`、空tail IDs和零资源只保留protocol attempts，resume不构造provider。已有trace和terminal call不重复。`_pre_flash_terminal_root_facts()` 与 `reproject_pre_flash_exp2_terminal_root_context()` 仅对一个冻结 Exp2 key，从既有 terminal ledger/artifacts/trace 重建 fixed-response typed facts并写projection，明确不重跑protocol、provider或transport；重复submission、非终态或任何非exact cohort均拒绝。来源有效`no_final`继续tail，tail资源只计真实provider calls，不建立第二runner或状态机。 |
| `experiments/slim_v2/projector.py` | 从同一次system result、ledger、worker facts、plugin/checker与Slim route artifacts投影唯一`RootResultV2`；`failure_origin`始终出现且可null，root顶层只用三分类。`_natural_rejection_stage()` 额外识别唯一 `lease_expired` 后的 `rejected/succeeded/lease_deadline_exceeded` submission 为既有 `child_execution` 自然拒绝，其他自然恢复仍严格要求原失败事实。最终attempts统一合并protocol snapshot与已持久化tail事实，使fresh/resume对相同事实生成相同typed结果。 |
| `experiments/slim_v2/schema.py` | Slim root/attempt/fault/recovery/death/challenge/ablation typed schema与跨字段不变量；ordinary root唯一生产schema为`tokenshare.slim_v2.root_result.v2`，`failure_origin`必现可null，v1明确拒绝且无migration/compat/dual reader；Exp1 tail字段始终必填，空tail合法状态区分`not_needed`与非来源的`not_required_by_downstream`，后者要求空IDs/零资源；`verified_correct`与`failure_kind`双向互斥。`PRICING_VERSION`/`LEGACY_PRICING_VERSION` 与 `PRICING_VERSIONS` 明确前向 Exp1 Flash=`2026-08-23`、Exp5=`2026-08-20`，并只定义 exact pre-Flash run/root/config identity 供窄恢复选择。 |
| `experiments/slim_v2/storage.py` | 普通run文件与resume事实；只读写strict-v2 root result与embedded protocol projection。来源Exp1 snapshot冻结protocol projection/traces、deterministic tail requests及已知pre-dispatch tail traces；resume从当前root已持久化trace闭合缺失tail。非来源Exp1 snapshot以typed base projection的`not_required_by_downstream`政策事实允许空tail material，并禁止携带tail request/pre-dispatch trace；CLI预检从该冻结状态判定无需provider。非Exp1仍禁止tail字段/trace。 |
| `experiments/slim_v2/provider.py` | single-entry 16MiB bounded provider调用、usage/model/cost普通投影和intent/response/typed-terminal journal；`project_cost()` 对前向 `deepseek-v4-flash` 使用独立 `0.05/1.50/4.50` flat 公式和`2026-08-23`版本，对旧 Pro 只保留 historical peak/off-peak 事实、对 Exp5 保留 SiliconFlow `2026-08-20`。恢复只读既有事实，dead intent写unknown且不重复同ordinal。 |
| `benchmarks/paper/exp1_baseline_provider_config.v3.json` | 前向 Exp1 tracked 单 entry config：`deepseek_v4_flash_exp1_baseline`、`deepseek-v4-flash`、thinking/high、600秒、300000 tokens 与 Flash 用户冻结价格常量；它不含 secret，也不构成价格门禁。 |
| `experiments/slim_v2/reducer.py` | 唯一Exp1–5离线统计内核，只接受strict `RootResultV2`。按table/slice流式归约，拥有固定分母、153 formal occurrences之外的三项mandatory root库存计数、pair/quadruple、type-7、sample variance、固定bootstrap与null finalizer。`_simulated_attempt_cost()` 按 source attempt 的 `pricing_version/pricing_tier` 分开使用 Flash flat 或 legacy Pro 价格，而不把 Pro 公式套到 Flash。Exp1 任一协议 provider usage 缺失时，token/cost total 保持`null + usage_missing`，不当成0且不丢失完整 latency。Exp5 对既有 `not_applicable_or_unavailable` 标记的 `2xx/raw/parsed` 未提交事实保留调用、覆盖和资源，内部作为不可评估：不伪造三类失败或验证拒绝，nonpass及其区间为`null`；同 cell 又含infra-invalid时，nonpass N/A 优先保留，验证拒绝率与调用覆盖率写`infrastructure_invalid_root_present`。验证拒绝率的 bootstrap 仅采样有实际 checkable candidate 的 root。仅 repeat0 的墙钟输出保持既有字段和缺失原因。正式point ratio由统一value/reason路径写入，零分母固定`null + zero_denominator`；三类failure breakdown只按committed `failure_kind`直接分组，`failure_origin`仅作诊断；infra-invalid仍保留输入完整的精确计数与资源事实，只使相关科学rate/effect/interval为null。生产`reduce_run()`同时服务真实运行与Representative fake E2E；独立`reduce_exp5_with_exp1_v4_reference()`只读三模型 Exp5 target 与同 case Exp1 Flash V4 source，原子发布supplemental JSONL/CSV，不触碰正式五表/summary，且V4所有wall-clock均为N/A。 |
...
| `experiments/slim_v2/cli.py` | 唯一实验编排入口；实现`plan/run/run-all/reduce/representative/lean-environment-test`、串行roots、strict-v2 typed resume、显式source与condition失败作用域。`compare-exp5-v4-reference --run-dir --source-run-dir`是唯一额外的零调用比较入口，直接调用补充 reducer 而不启动runner或默认services。前向 profile/config 走 Flash；`_is_pre_flash_representative_resume_request()`、`_validate_pre_flash_representative_resume_config()` 与 `_run_experiment_locked()` 只为 exact historical cohort 接受旧单值 config，并只分派唯一 Exp2 terminal-ledger reproject，绝不放宽为一般 compatibility。Exp2–4 source run必须具有strict-v2 committed Exp1 roots，v1/旧run不兼容也不迁移；`representative`按冻结profile走production execute/runtime/storage与真实`reduce_run()`，不是独立runner。 |
...
| `tests/experiments/slim_v2/test_answer_paths.py`、`test_profiles.py`、`test_schema.py`、`test_cli_e2e.py` | 覆盖 Flash entry/model、前向 `pricing_versions`、`project_cost()` flat 公式与 CLI/provider identity，且保留 exact historical Pro 价格事实的非前向分支；`test_profiles.py` 还锁定 Full Exp5 28+3×3/仅repeat0/零重试、852 online upper，以及 Full 84,618 fixed / 91,380 total protocol attempts。 |
...
| `tests/experiments/slim_v2/test_reducer_golden.py` | 单一golden run风险驱动覆盖153/153、fixed denominator、tail、独立pair eligibility、六组四端、Exp5分类、bootstrap/null阈值与发布事务；另覆盖strict-v2读取、Exp1 protocol usage缺失时token/cost的`usage_missing`、Exp2 stale reason清理、Exp3/4/5正式零分母reason、committed failure taxonomy，以及infra cell保留精确事实和pair reason传播。Exp5额外覆盖 parsed-but-unsubmitted 的可评估性边界、其与infra-invalid同cell时的reason优先级、缺既有N/A标记时fail-closed、仅repeat0墙钟兼容，以及C root不污染可检查验证拒绝率的bootstrap；还覆盖补充表保留三个实测模型的wall-clock、而Exp1 Flash V4只保留质量/token/成本且wall-clock全为N/A。 |
| `tests/experiments/slim_v2/test_runtime_resume.py`、`test_scenarios.py` | 覆盖从protocol snapshot与当前root全部持久化coverage-tail traces稳定重建attempts；fresh/resume typed结果相等，完整tail调用`3→3`、部分tail`2→3→3`，不重复已有trace或terminal ordinal。另以 `625,812 ms` source latency 覆盖 lease-expiry 后三个 `rejected/succeeded` submissions 的投影；exact terminal-ledger reproject 显式禁止 transport/provider、拒绝重复 submission 与非终态，并验证 CLI frozen selection 绝不扩展。 |
| `tests/experiments/slim_v2/test_cli_e2e.py` | 全离线覆盖原子命令、source/failure scope、run lock、strict-v2 source/resume及生产Representative fake E2E；冻结profile提交71 paper roots+2 references，唯一fake网络边界为provider `_open_response`，真实`reduce_run()`发布summary与Exp1–5 JSONL/CSV共11文件；补充比较CLI不启动runner/provider。 |
