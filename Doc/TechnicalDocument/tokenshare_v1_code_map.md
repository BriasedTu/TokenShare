...
## 在制里程碑：representative full-plan smoke（未提交）

- `src/tokenshare/experiments/paper_formal_plan.py`：完整正式 snapshot/prepared inventory、repeat/root-filter representative coverage，以及 Factor worker-death root 的正式 partition/verifier viability；`FormalPreparedRequestInventory.to_dict/from_dict`、`persist_formal_prepared_request_inventory` 与 `load_formal_prepared_request_inventory` 提供 canonical JSONL+manifest 持久化与 strict reload，load 不调用 adapter/prepare/freeze。不得在这里创建 smoke-only condition/selection/seed/split。
- `src/tokenshare/experiments/paper_response_bank.py`：从经验证 full authority 派生 exact supplemental requests、natural identity dedupe、formal bundle lineage 与 atomic acquisition authority；`prepare_results_first_acquisition_authority` 必须绑定同一 typed full prepared inventory，request/body/provider identity 与 hard ledger 不得放宽。
- `src/tokenshare/experiments/paper_formal_runner.py`、`run_paper_experiments.py`：完整 plan/budget 先验后 selected-condition/root dispatch、Exp1–4 trace/Exp5 online 接线、formal trace inventory identity 与指标汇总。
- `src/tokenshare/experiments/factorization_paper_adapter.py`、`lean_paper_adapter.py`、`paper_faults.py`：raw-before-checker fault、formal endpoint entry identity、full Lean certificate DAG worker-death projection，以及合法但 checker-rejected 的 false-negative payload。
- `src/tokenshare/local_runtime/workers.py`：真实 process completion/progress facts；`coordinator.py`：从 checkpointed `retry_counts + READY state` 派生 recovery-ready obligation，保证 replacement 完成后才允许 merge/prune，同时不改变 raw witness 指标。
- `tests/experiments/test_results_first_full_plan_smoke_e2e.py`：唯一新的 full-authority 145-root offline representative E2E；final-14 执行主体只读保留，fresh metrics-only-9 仅表示 offline metrics closure/验收边界 PASS，未重跑 E2E 且 offline 里程碑不标记 completed。上述在制文件未授权 stage/commit，详情以 `session-handoff.md` 为准。
...
| `paper_formal_runner.py` | 正式/smoke suite orchestration、preflight、dispatch、checkpoint、resume/replay；results-first 默认 `enforce_publication_closure=False`，只跳过设施 publication closure，仍走原 ProtocolEngine、正式 adapter/coordinator、领域 verifier/checker 与 metrics；历史 strict publication 路径显式传 `True`。Task 22 的 trace 仍按 condition/per-worker lane 聚合 logical runtime。2026-08-19 增加两个既有 protected closure 的独立 evidence materialize/rehydrate、按 root strict merge 与统一 metrics helper；固定分区=`Exp1/2/3/5:12/6/81/16`。Exp5 replay 的 endpoint identity 只能由已验证的 `PaperModelExecutionRecord` v2 artifact 重建，不能信任 producer/pickle facts。R13 response bank 仅供 Exp1–3 trace；Exp5 loader 从当前 provider config 重建 binding 与 pricing authority，不继承 R13 旧价格。 |
...
| `paper_exp1_trace_reuse.py` | Exp1 source-attempt authority、Exp1 protocol finalizer 与 Exp2–4 trace reuse seam；source bank 只作为合法性查表，finalizer 按本轮实际 committed source entries 分类，未观测 entries 保持未分类。response identity 按 outcome 对齐：成功 attempt 对比唯一 `raw_output` 与 authority raw digest，provider failure 对比唯一 `provider_failure` locator 与 authority raw digest；每个 committed delivery 的 formal `raw_output_ref` 必须等于 current wrapper ref。wrapper/binding/digest/task/event/ledger identity 仍严格核验，不改变后续 root-level closure 或 metrics 分母；定向回归位于 `tests/experiments/test_exp1_trace_reuse_semantics.py`。 |
| `paper_ablation.py` | `FULL + 4` protocol mechanism policy。 |
| `paper_terminal_outcomes.py` | succeeded/failed/blocked/incomplete 终态语义。 |
| `paper_smoke.py` | smoke profile、identity 与非论文执行；双域 profile 固定恰好 `factor_v2_easy_109` + `lean_easy_01` 两个 roots，复用 production runner/adapters/checker/metrics，分类保持 regression-only/pilot/paper-ineligible。 |
...
| `paper_historical_fixture.py`、`historical_real_factorization_single_leaf.py` | 历史真实 Factor single-leaf 离线回归：manifest 声明的 JSON 文本在 digest 比较时确定性规范化 CRLF→LF，孤立 CR、内容差异及非文本 source 仍 fail closed；运行继续经正常 coordinator/parser/verifier/canonical/merge/ledger 与 central direct projector，provider=0。 |
...
| `paper_report.py`、`paper_formal_report.py`、`paper_smoke_report.py` | 通用/正式/smoke 输出；smoke schema v2 保持旧字段并在 summary/rows/CSV 增加 correctness、completion 固定 roots 分母，以及经 `model_execution_record_ref` 绑定的 provider latency total/sample/missing；缺失 latency 不补 0，token/cost 保持 attempt 实值。smoke 永远 paper-ineligible。 |
...
### 实验 readiness repair 与当前运行边界

截至 2026-08-09，Factorization catalog=`500`、Lean lemma catalog=`165`。results-first 修复已让 canonical 双域 smoke 继续复用原 TokenShare 生命周期；publication/receipt/L1–L4 属非指标设施 closure，可在 results-first 默认路径跳过，不能借此绕过 ProtocolEngine、adapter/coordinator、parser/verifier/checker、accepted 或 metrics。需要旧 publication 行为时使用显式 strict opt-in。

已验收真实 smoke root=`E:\TokenShareData\outputs\experiments\dual-domain-smoke-real-fixed-20260808-184048`：2 roots、4 次 HTTP 200 DeepSeek attempts、tokens=`93859`、cost=`0.527757 CNY`、provider latency=`920236ms`、correctness=`1/2`；Factor false 保留真实失败，Lean 2 child + 1 merge 均由 checker accepted。该 profile 仍为 paper-ineligible，不冒充正式论文矩阵。

正式全量矩阵尚未运行；只可在用户另行授权预算后从 results-first 入口执行。指标必须保留固定 roots 分母、False 与 None 区分、真实 attempt latency/token/cost，且可回到逐 task/attempt/event/artifact；缺失不得补零或伪造成功。历史 Full 不作为本次 readiness 验收标准。

### CLI

| 文件 | 默认输出类别 |
|---|---|
| `run_paper_pipeline.py` | 统一正式命令的 parser/delegation/门禁入口；历史 strict publication 流仍可显式 opt-in，unknown/missing 为 exit 2，正式资源阻塞为 JSON/exit 3。`persist_results_first_paid_restore_binding_v2` 只写已有 typed artifact 的 overlay binding，`load_results_first_paid_execution_authority_v2` 从 frozen snapshot/inventory/bundle+preflight 的 strict persisted authorities 恢复 paid authority，禁止将 restore target 与 frozen source 混用或在 restore 时重跑 preparation。2026-08-19 新增纯离线 `audit-results-first-metric-merge` 与 `render-results-first-combined`：前者复验双 protected closure 的固定 115-root audit，后者复验该 audit 后生成 fresh combined metrics/report；均拒绝 receipt/profile/transport/provider 输入。 |
| `run_paper_experiments.py` | results-first 默认不强制 publication closure，仍委托同一 authority/service map 与 production ProtocolEngine/adapters/checker/metrics；必须使用全新 `--output-root` 和用户批准的 calls/tokens/cost 硬预算，不能用 unlimited budget。 |
...
| `local/full_exp1_exp3_exp5_audit.py`、`local/start_exp1_exp3_exp5.ps1 -Mode FullAudit` | Exp4 排除的 Full 零调用准备入口：只接受 `full_exp1_exp3_exp5`，从冻结 Full snapshot/inventory 派生新 Full Exp1 pending-acquisition typed bundle、Exp5 online projection、CNY 10000 外层门和 scope-bound FullRun gate；禁止 R13 90-entry bank，任何输入/digest/fresh-root 漂移均 provider 前阻断。`FullRun` 只接受该 gate 的 exact binding。 |
| `local/Newfullrun.ps1`、`local/newfullrun_audit.py` | 新的 Full 第一阶段 current-pricing 预算审计。固定 `full_exp1_exp3_exp5`，以 Full coverage 重新计算 Exp1 acquisition + Exp5 online（Exp2/3 trace replay 为 provider=0），并从当前 Exp5 provider config 重建 pricing authority。只写 fresh audit 与 Full budget approval authority；cap 内状态仅为 `awaiting_user_approval`，receipt 始终未签，绝不创建 bank、dispatch、调用 provider 或启动 Full。 |
| `local/Newfullrun_execute.ps1`、`tests/experiments/test_newfullrun_scripts.py` | 单一 A 批准后的 Full 编排器：自动派生 internal acquisition/execution receipt，依次运行 bank acquisition、scope preparation、provider-zero plan-only 与 paid canonical `run-results-first --selection full_exp1_exp3_exp5`；loader 直接校验 A/current-pricing/bank manifest lineage。 |
