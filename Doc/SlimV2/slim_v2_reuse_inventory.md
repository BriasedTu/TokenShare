...
| 条件复用+获批最小扩展 | `src/tokenshare/local_runtime/contracts.py:809-906` directives、`RuntimeHooks`、`NoOpRuntimeHooks` | 提供 raw/parsed/parser/verification/requeue/normal-merge/progress 的稳定接缝 | runtime context | bypass/result-kind/worker directive 或无动作 | 既有normal `before_merge`太晚，不能复用为Exp4 M；按获批设计只增加独立optional `RecoveryMergeContext` capability，默认caller不实现所以零新增调用 |
| 直接复用 | `src/tokenshare/local_runtime/contracts.py:910` `ProtocolMechanismPolicy` | 控制 parser policy、verification、replacement、merge 等协议机制 | 五个普通布尔值 | coordinator 使用的机制配置 | 这是 Experiment 4 的配置自变量，不证明实际route执行；P/Lean-V仍需Slim-local调用前旁路，M需获批recovery-premerge seam |
...
场景层仍需新写一个很小的 `ScenarioConfig -> (worker_backend, logical_scheduler, mechanism_policy, challenge_controller, structural_route_observer)` 映射函数。它只翻译权威条件，不判断“这组结果是否可发表”。challenge controller不可见mode/policy，structural observer不可决定challenge。Experiment 4 当前是 FULL、四个单机制和六个双机制共11 modes；每个双机制mode必须在同一次root run中同时关闭对应两项，不能沿用旧四/五模式表。

五类 fault 存在一个必须明确的接缝：raw-output fault 发生在 executor 一侧，false-positive/false-negative 发生在 coordinator `RuntimeHooks` 一侧。Slim 需要一个共享计数状态的轻量 `ScenarioFaultBridge`，一端是 executor raw callback，另一端实现 parsed-candidate hooks；不得复用会生成 artifact/digest observation 的 `PaperFaultRuntimeHooks`。Experiment 4只可借`paper_ablation.py:421-452`的单机制映射思想，不能复用其evidence hook；P必须在Slim fixed adapter `_parse_domain`前旁路，Lean V必须在Slim bridge调用`normalize_proof_submission`前旁路，M使用获批optional recovery-premerge seam和Slim-local v2 observation。
