...
状态：支持性审计草案（不是 Slim V2 设计规格）

审计日期：2026-08-20

审计对象：当前 `exp-full-run` 工作树中的源码；行号以本次审计时文件内容为准。
...
后续 Agent 不应先通读旧实验文档或 `paper_*` 设施。应先读上述三个权威文档，再读本文，并且只打开本文点名的源码位置。
...
| 只借局部逻辑 | 所在文件过大或依赖了门禁/证据链 | 阅读指定函数，复制并化简必要算法；禁止 import 整模块 |
| 禁止复用 | 会把旧 formal/paper 设施重新带回来 | 不读取其实现来驱动设计，不建立依赖 |

“直接复用”优先表示复用公开符号，不表示复制整个文件到 Slim V2。对于系统本体的大模块，Slim V2 只从公开边界调用，把它视为外部黑盒。
...
当前两个 plugin runtime 都在模块顶层从 `executors/ai_api.py` 导入 `build_ai_api_executor_descriptor`（factorization `runtime_adapter.py:23`，Lean `runtime_adapter.py:21`）。这个描述符函数本身很小，但所在文件有 2156 行。若 Slim V2 的目标只是建立一条独立轻量实验入口，可以暂时接受该系统内部依赖；若后续目标升级为物理删除旧代码，应先把 descriptor builder 移到独立小模块，再删除巨型 AI executor，不能直接删文件。
...
| 边界隔离 | `src/tokenshare/executors/contracts.py:188-239` `ExecutionRequest` / `ExecutionSubmission` | 系统 worker 的既有执行合约 | protocol request、artifact refs | protocol submission | `slim_ai.py` 不依赖它；只有连接系统 worker 的小 `SlimExecutionBridge` 把 `SlimAIResult` 转成 submission |
...
2. 阅读本文第 2 节，确认无门禁边界。
3. 按“系统对外接线契约”只打开第 5 节列出的公开系统入口和插件 runtime。
4. 按论文指标清单确定每个 root 必须保存的最小原始字段。
5. 从本文“直接复用”项建立薄接线；对“只借局部逻辑”项只阅读指定行附近，不 import 模块。
...
本次只读审计没有调用真实 API，没有运行 R52/R53/R54，没有读取其运行证据，也没有运行测试；测试位置表示已有源码证据锚点，不表示本轮重新验证通过。除本文和同轮 Slim V2 文档修订外，没有修改系统或实验代码。三个前置权威文档现已获批并冻结；本文仍只是一份支持性复用清单，不冻结 Slim V2 的最终接口和设计规格。官方价格来自另一次只读官网核对，详见 `Doc/SlimV2/slim_v2_official_pricing_sources_20260820.md`。
