...
- **选择性Exp1 coverage policy**：同一 profile 的 Exp2、Exp3、Exp4（包含它们实际调度的 reference）将消费 trace 的 `case_id` 并集是唯一的tail-required集合；它由冻结inventory实际派生，Full当前验证为99，禁止硬编码该数。只有该集合中的Exp1来源root须为全部planned unit形成strict trace closure；非来源Exp1 root在任何已取得的protocol terminal后都不得调用tail，必须以`trace_tail_status=not_required_by_downstream`、空target/recorded IDs和零tail资源提交，同时保留protocol attempts及`unscheduled_ai_unit_ids`。此政策独立于condition/runtime/infrastructure tail blocker；blocker只阻断来源root的tail。Exp5 V4 supplemental只读committed `RootResultV2`，不读`UnitTrace`，绝不加入集合。
