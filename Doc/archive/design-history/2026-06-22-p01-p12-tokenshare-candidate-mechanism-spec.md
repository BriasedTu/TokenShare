...
2026-06-28 Lean 范围覆盖：旧段落中“Lean stub / synthetic fixture only / 真实 Lean checker 留到后续”的口径已废弃。Phase 6 必须实现本地真实 Lean checker 驱动的形式化证明插件；仍不实现生产级 theorem-proving 平台、LeanDojo 训练/检索平台或动态 Lean 服务。若本文旧裁决与此冲突，以主 TDD 和 `2026-06-28-phase-6-lean-real-plugin-scope-change.md` 为准。
...
| P19 LeanDojo | proof state、tactic、next/error/done 交互；精确 proof environment；premise retrieval 与 best-first search | 约束真实 Lean 插件的 checker-authoritative、environment-bound 验证、diagnostics 和 replay 边界 | V1 实现 DPR、模型训练、真实 premise retrieval 或完整 proof search 平台 |
| P20 miniF2F | 固定 benchmark 版本、问题 ID、验证/测试划分、题型与难度覆盖 | 指导 Lean 实验 fixture / benchmark 子集的版本化、分层和可比较性 | 强制 V1 下载完整 benchmark、把 benchmark 写进协议核心、以 benchmark 分数替代协议验证 |
| P21 mathlib | Lean/mathlib 的 library、automation、结构层级与版本演进背景 | 说明 proof checker 依赖具体 library/toolchain 语境，辅助定义真实 Lean `EnvironmentRef` | 将其误写为 environment hash/replay 算法来源；把完整 mathlib 平台或服务塞进协议核心 |
...
1. factorization、真实 Lean 形式化证明插件和 structured report stub 都通过同一协议接口执行，核心中不存在按插件名称分支的领域逻辑。
...
- Lean 形式化证明：proof artifact 在 request 绑定的固定 Lean/lake/toolchain/library environment 下通过真实 checker；
...
- `proof_checked`：候选携带可由固定版本 checker 重放的证明或 proof-like artifact，例如真实 Lean 形式化证明插件。
...
5. factorization 和真实 Lean 形式化证明插件各自只凭一个通过固定领域 checker 的候选即可进入 selection，不要求重复答案一致。
6. structured report 的 schema/引用检查通过时，报告只能标记为“通过有界结构验证”，不得记录为“语义真值已证明”。
7. 使用错误或不同版本 verifier environment 检查 Lean proof artifact 时，validation 明确失败或报告环境不匹配，不能归因于 proof 本身。
...
5. Lean 报告可精确列出 Lean toolchain、lake project、library/import set、benchmark/problem IDs 或 fixture case IDs；同名输入或环境内容变化会产生新 digest。
...
7. environment-sensitive validation 绑定不可变 `EnvironmentRef`/digest；真实 Lean checker 与 validation 使用同一环境身份。
8. append-only event ledger、不可变 artifact 和零执行 state replay。
9. factorization、真实 Lean 形式化证明、structured report stub 三条完整实验链。
...
7. LeanDojo/premise retrieval/best-first proof search 平台和生产级 Lean 服务；Phase 6 真实 Lean checker 插件可以使用固定本地 toolchain / lake project / library context，采用 benchmark 时固定版本、problem IDs、split 和环境，不写入协议核心。
...
| 实验与证明 | 把真实 Lean checker 插件扩成 LeanDojo/DPR/真实 proof search 平台 | 会把协议 PoC 扩成 theorem-proving 平台项目 | Phase 6 只要求本地真实 checker、确定性拆分、artifact/replay 边界；premise retrieval/search 平台不进入 V1 |
| 实验与证明 | 把 miniF2F 完整 benchmark 当 V1 必需依赖 | 四天论文窗口下完整 benchmark 会引入数据、toolchain 和维护成本 | 可采用小型真实 Lean fixture 或受控 benchmark 子集；若采用实际子集则固定版本和 problem IDs |
...
截至 `candidate-baseline-v3`，第一部分形成一套统一候选架构：TokenShare 以“动态 typed task DAG + 私有多 attempt + 分层验证 + 唯一 canonical bundle + immutable artifacts + append-only events + 验证后结算”为协议骨架；递归拆分通过受控扩图表达，执行授权通过 lease/fencing 表达，分派通过 capability filter 和可解释的 `AllocationDecision` 表达，领域行为与搜索策略通过版本化插件表达。验证结论必须与选择决定、控制面排序和统计估计相互分离；proof-like 结果绑定精确环境，工具型执行保留 action/observation provenance，可复现实验固定 profile、fixture 和实际随机决定。真实分布式协商、长期信誉和通用搜索 runtime 保持为后续扩展；本地真实 Lean checker toolchain 已按 2026-06-28 范围变更进入 Phase 6。
...
- 已有 `Attempt.environment_summary` 和版本化 checker 方向，为真实 Lean `EnvironmentRef` / environment-bound validation 提供了基础。
...
- factorization 与真实 Lean 形式化证明插件应坚持强 checker 优先；Dawid-Skene、GLAD、MACE、CROWDLAB 只能作为满足各自离散标签、交叠观察、difficulty/spam/classifier 等前提的未来插件，不能合并成通用 semantic score 或永久信誉。
- 真实 Lean 插件必须统一版本化 benchmark/problem IDs/split 和环境身份。miniF2F 可指导真实 fixture / benchmark 子集设计；mathlib 提供 library/version 背景；environment-correct checking 的交互边界主要参考 LeanDojo。
...
| `TS-OBJ-005` | 部分覆盖 | `Attempt.environment_summary` 和 Lean checker 环境摘要方向已存在 | 没有不可变 `EnvironmentRef`/digest 的最小 schema，也没有 request/submission/validation 一致性检查 |
...
| `TS-VERIFY-011` | 部分覆盖 | Lean 目标包含固定环境摘要、版本化 checker 和 proof artifact | validation 未绑定精确 environment identity；audit verification 换环境后的新记录/不覆盖规则未写明 |
...
| `TS-INV-017` | 部分覆盖 | Lean 插件已要求固定环境摘要和 checker | 环境仍是自由结构摘要，缺少不可变 identity 与跨 request/submission/validation 的一致性约束 |
...
这组裁决区分执行授权、审计证据、验证结论和正式输出，并确定真实 Lean 形式化证明插件的实验边界。
...
- **方案 C**：仅 Lean 插件使用固定字符串，其余 task 不定义环境合同。
- **推荐**：方案 B。P19 已证明细微 namespace/import 环境差异会让正确 proof 被误判；统一引用也能覆盖 deterministic executor、真实 Lean checker 和未来容器环境，而不要求 V1 引入容器或生产级 Lean 服务。
...
- **推荐**：方案 B。factorization 与真实 Lean 形式化证明插件使用强 checker；structured report 只声明其有界结构验证范围，不把 schema/引用通过夸大为语义真值。方案 A 会把一致性错当正确性，方案 C 缺少适合的 V1 数据、独立观察者和研究必要性。
...
#### `DEC-P19-022`：真实 Lean 插件采用何种交互粒度

- **方案 A**：V1 使用真实 Lean checker 检查完整 theorem/proof artifact，插件确定性拆分只在 canonical 输出后递归生成子任务。
- **方案 B**：V1 实现 `initialize(theorem)`、`run_tactic(state,tactic)`、next/error/done 的多步 proof-state 交互，但仍只做本地 checker，不做 LeanDojo 训练/检索。
- **方案 C**：直接实现 LeanDojo/DPR/premise retrieval/best-first search 平台。
- **推荐**：方案 A。它能在论文窗口内给出真实 checker 能力，同时保留协议的 artifact、verification、expansion、merge 和 replay 边界；方案 B 可作为后续增强，方案 C 仍超出 V1。
- **影响阶段**：Phase 6 Lean 插件字段规格与实验验收。

#### `DEC-P20-025`：Lean 插件是否使用实际 miniF2F 子集

- **方案 A**：V1 下载并固定一个 miniF2F/Lean 子集，使用真实 statement/problem IDs 作为 fixture。
- **方案 B**：V1 使用仓库内真实 Lean theorem/proof fixture，不下载完整 miniF2F，但借鉴 miniF2F 的难度/题型分层和版本固定方法。
- **方案 C**：两者都做，以小型本地真实 Lean fixture 验证协议，以实际子集补充 benchmark 报告。
- **推荐**：方案 B。当前论文窗口更需要证明真实 checker 与协议闭环，而不是完整 benchmark 分数；若采用实际子集，必须固定版本、problem IDs、split 和本地材料索引。
...
4. **在实现验证前分清四类决定**：`DEC-P01P07-007`、`DEC-P08P15-020`、`DEC-P08P12-019` 与 `DEC-P01P07-008` 分别处理领域正确性、统计估计、控制面阈值误映射和正式选择；再用 `DEC-P19-022`、`DEC-P20-025` 固定真实 Lean 插件与 benchmark 边界。
...
- Phase 6 只实现 factorization、真实 Lean 形式化证明和 structured report stub 三类实验插件及其插件内 schema、validator、split strategy 和 merge policy。
...
- Lean 插件明确采用真实 Lean checker 的 direct proof / decomposition 路径，并固定 Lean toolchain、lake project、library/import set、namespace/context、fixture/benchmark profile；LeanDojo 训练/检索平台保持为后续工作。
...
现有主 TDD 与 P01-P22 独立推导出的协议骨架总体同向。已有设计已经正确建立协议核心、插件和执行器分层，隔离 candidate 与 canonical output，以受控 proposal 扩图，以 append-only event 和 immutable artifact 保存事实，并把真实分布式 runtime、生产 agent 系统和生产级 theorem-proving 平台留在 V1 之外；但 2026-06-28 起，真实 Lean checker 驱动的本地形式化证明插件进入 Phase 6。需要加强的不是组件数量，而是若干跨组件合同：expected output/resolution、requirements/hints、selection identity、verifier environment、durable state promotion、action/observation provenance、可解释分派和可复现实验字段。

论文提供的算法必须服从这些合同，而不能反向塑造协议核心。强 checker 仍是 factorization 和真实 Lean 形式化证明插件的正确性来源；统计弱验证只在满足具体模型假设的离散任务中启用；ToT、ReAct 和 decomposer 只能作为插件内部策略或执行器候选输出生成方式，协议级拆分规则必须属于版本化插件；miniF2F 和 mathlib 约束真实 Lean fixture、benchmark 子集与环境版本；Contract Net 只贡献 eligibility、assignment 和 refusal 语义；Raft/PBFT 只约束未来控制面排序。由此，TokenShare V1 可以验证递归拆分、执行、验证、合并、结算和 replay 闭环，而不被任何一篇论文的完整系统范围拖走。
