...
3. Task 0–16 已 accepted；使用已完成的 Task 17 只读预检，按单写入者 TDD/review 循环推进 Task 17，不得进入 Task 18。

## 2026-08-02 EPD-027 当前实施交接（Task 16 accepted）

- Active feature=`feat-011`，仍为 `in-progress`；active focus id 保持 `paper-metric-contract-trace-bank-and-traceability-plan`，status=`in_progress`。Task 0–16 已 accepted（`17/35`）；Task 17 为 `queued/next`；正式矩阵仍为 **NO-GO**。
- 当前分支=`codex/feat-011-epd027-pipeline`；Task 16 projector commit=`440c1ef9`，状态提交在其后。
...
- Task 16 reviewer BLOCK=`1 Critical/2 Important`；targeted RED=`5 failed in 0.36s` + `1 failed in 0.48s`，最终 review canonical=`9 passed in 1.20s`。final reviewer PASS=`0/0/0`；子智能体 test minimization=`CHANGED` 后 canonical=`9 passed in 1.17s`；provider/network=`0/0`。
- 用户已授权后续每个 Task 持续实施/review/fix/follow-up 直至 PASS 后直接推进，且 post-accept 测试最小化必须由子智能体执行。下一动作仅为 Task 17 registry/formal metrics 四文件；不得进入 Task 18。
...
- 在该历史节点只完成 `design_synced`，尚未实现或调用真实 API；此后完整实施计划已形成，当前 Task 0–16 已 accepted。不要再把本条历史快照当作当前状态。
...
- Active feature：`feat-011`。EPD-027 accepted 进度保持 `17/35`：Task 0–16 已 accepted；Task 17 为 queued/next，Task 18 及以后保持 queued。immutable bank、inventory/preflight、budget、acquisition、logical scheduler、parent-owned commit ABI、trace-backed executor / dual provenance 与 standalone Exp1–5 projectors 已 accepted；registry 与 formal runner/metrics/renderer/replay 接线仍未闭合。
