...
from tokenshare.experiments import lean_environment
from tokenshare.experiments.lean_environment import (
...
def test_startup_validation_is_lightweight_and_fails_closed_on_object_drift(
    ...
    object_path = repo / "benchmarks/experiments/fixtures/lean_proof_project/.lake/build/lib/lean/TokenShare/LemmaGraphOracle.olean"
    ...
    cases_object = repo / "benchmarks/experiments/fixtures/lean_proof_project/.lake/build/lib/lean/TokenShare/LemmaGraphCases.olean"
    cases_object.unlink()
...
def test_default_lean_library_imports_the_oracle_module() -> None:
    source = (
        _REPO_ROOT / "benchmarks/experiments/fixtures/lean_proof_project/TokenShare.lean"
...
def _fake_project_builder(repository_root: Path) -> tuple[Path, Path]:
    object_dir = (
        repository_root / "benchmarks/experiments/fixtures/lean_proof_project/.lake/build/lib/lean/TokenShare"
    )
...
def _minimal_repository(tmp_path: Path) -> Path:
    repo = tmp_path / "repo"
    project = repo / "benchmarks" / "experiments" / "fixtures" / "lean_proof_project"
    ...
    catalog = repo / "benchmarks" / "experiments" / "lean_lemma_graph_catalog.v1.jsonl"
