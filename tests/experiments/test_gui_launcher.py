...
def test_build_cli_argv_maps_all_twelve_core_selections(
    ...
    from tokenshare.experiments.gui import build_cli_argv
...
def test_single_process_launcher_blocks_duplicate_until_exit() -> None:
    from tokenshare.experiments.gui import _SingleProcessLauncher
    ...
    launcher.start(["python", "-m", "tokenshare.experiments.cli", "run-all"])
...
def test_opening_window_starts_no_cli_or_provider(
    ...
    from tokenshare.experiments import gui
...
def test_launcher_is_only_a_checked_gui_entrypoint() -> None:
    ...
    script = (repository / "run_experiments.cmd").read_text(encoding="utf-8").lower()
    ...
    assert script.count(
        ...
        "tokenshare.experiments.gui"
    ) == 1
    assert "tokenshare.experiments.cli" not in script
