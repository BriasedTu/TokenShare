...
def test_resume_protocol_tail_and_started_without_protocol_never_rerun_root(
    ...
    from tokenshare.experiments.schema import ExperimentRunConfigV1
...
@pytest.mark.parametrize(
    ...
    [
        ("condition-failure-resume", {"experiments_condition_failure": {"failure_kind": "x"}}),
        (
            "runtime-failure-resume",
            {"experiments_runtime_failure": {"failure_origin": "unexpected_runtime_error"}},
        ),
        (
...
def test_invalid_run_lock_is_fail_closed(tmp_path: Path) -> None:
    ...
    path = tmp_path / ".incomplete.experiments.lock"
...
def test_same_run_lock_blocks_before_store_or_provider_and_is_released(
    ...
    assert first_errors == []
    assert not (tmp_path / ".locked.experiments.lock").exists()
...
def test_next_root_disk_bound_is_dynamic_and_preflight_writes_no_run_dir(
    ...
    assert not (tmp_path / ".no-empty-run.experiments.lock").exists()
...
def test_representative_cap_secret_disk_and_price_are_run_safety_only(
    ...
    from tokenshare.experiments import cli
    from tokenshare.experiments.profiles import build_plan
    ...
    tracked = config_root / "configs" / "experiments"
...
def test_representative_fake_transport_uses_production_runtime_and_reducer(
    ...
    lean_fail_closed = [
        (inventory, result)
        for inventory, result in paper_results
        if result is not None and result.failure_origin == "lean_environment_invalid"
    ]
    assert len(lean_fail_closed) == 25
    assert {str(inventory.domain) for inventory, _result in lean_fail_closed} == {"lean"}

    # Clean checkout 没有 local Lean pass；Lean roots 在生产 preflight fail-closed，
    # 不进入 protocol execution。其余 root/reference execution 仍留下生产快照。
    assert len(list((run_dir / "roots").rglob("protocol.json"))) == 47
    assert len(list((run_dir / "system").rglob("events.jsonl"))) == 47
