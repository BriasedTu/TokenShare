...
_AUTHORITY_RELATIVE_PATHS = (
    ...
    "benchmarks/experiments/lean_checker_preflight.v1.json",
...
def test_environment_ref_records_legacy_and_semantic_authority_digests(
    ...
    _copy_semantic_authority_repository(copied_root)
    project_root = copied_root / "benchmarks/experiments/fixtures/lean_proof_project"
    ...
    case = json.loads(
        next(
            line
            for line in (
                copied_root / "benchmarks/experiments/lean_lemma_graph_catalog.v1.jsonl"
...
def _copy_semantic_authority_repository(
    ...
    *,
    ...
    _copy_project(
        _REPOSITORY_ROOT / "benchmarks/experiments/fixtures/lean_proof_project",
        destination / "benchmarks/experiments/fixtures/lean_proof_project",
    )


def _rewrite_semantic_sources(repository_root: Path, *, newline: str) -> None:
    project_root = repository_root / "benchmarks/experiments/fixtures/lean_proof_project"
    paths = [
