...
$ExpectedProviderScope = "epd027_l3_capability_and_online_checks"
$receipt = Get-Content -Raw -Encoding UTF8 -LiteralPath $PaidReceipt | ConvertFrom-Json
if ($receipt.scope -ne $ExpectedProviderScope) {
    [Console]::Error.WriteLine("paid receipt scope mismatch for EPD-027 L3 checks")
    exit 3
}
