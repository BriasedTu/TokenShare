"""Slim V2 的独立 Lean 环境实测与轻量 pass 文件校验。"""
...
DEFAULT_LEAN_ENVIRONMENT_PASS_PATH = Path(
    "local/cache/slim_v2/lean_environment_pass.v1.json"
)
_CATALOG_PATH = Path("benchmarks/paper/lean_lemma_graph_catalog.v1.jsonl")
_PROJECT_PATH = Path("fixtures/lean_proof_project")
...
def collect_checker_backed_nodes(
    ...
    for row in rows:
        ...
        if not source.is_file():
            raise LeanEnvironmentInvalid(f"{case_id} oracle source is missing: {source_path}")
...
def run_lean_environment_test(
    *,
    ...
    if node_checker is None:
        with tempfile.TemporaryDirectory(prefix="tokenshare_slim_v2_lean_environment_") as temp:
...
def _real_node_checker(
    ...
    environment_ref = EnvironmentRef(
        environment_id="slim_v2_lean_environment_test",
    ...
    def check(node: LeanEnvironmentNode) -> str:
        ...
        theorem_ref = store.save_json(
            ...
            source={"kind": "slim_v2_lean_environment_test"},
        ...
        report = check_lean_proof(
            LeanCheckerRequest(
                request_id=f"slim_v2_lean_environment:{node.case_id}:{node.node_id}",
