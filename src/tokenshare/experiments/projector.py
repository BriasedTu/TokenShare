"""TokenShare Experiments 单 root 只读投影。"""
...
def project_root_result(
    *,
    ...
    if inventory.experiment_id not in {"exp1", "exp2", "exp3", "exp4", "exp5"}:
        raise RootProjectionError("unsupported experiments experiment")
    ...
    if condition_failure is not None:
        if not isinstance(condition_failure, Mapping):
            raise RootProjectionError("experiment condition failure must be an object")
        if (
            ...
            raise RootProjectionError("experiment condition failure identity is invalid")
    ...
    *,
    ...
    for event in events:
        ...
        if not isinstance(summary, Mapping):
            continue
        parser_record = summary.get("experiments_parser_route_observation")
        ...
        actual_challenges = summary.get("experiments_challenge_observations", ())
