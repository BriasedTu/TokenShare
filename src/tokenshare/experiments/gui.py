"""把本地窗口输入确定性转换为唯一 experiments CLI 子进程。"""
...
_CLI_MODULE = "tokenshare.experiments.cli"
...
def build_cli_argv(
    *,
    ...
    """生成窗口当前选择对应的同一 experiments CLI argv。"""
    ...
    def start(self, argv: Sequence[str]) -> None:
        if self._process is not None:
            if self._process.poll() is None:
                raise RuntimeError("TokenShare Experiments CLI is already running")
            self._process = None
...
def _default_run_id() -> str:
    return datetime.now().strftime("experiments-%Y%m%d-%H%M%S")
...
def main() -> int:
    ...
    root = tk.Tk()
    root.title("TokenShare Experiments")
    ...
    def poll_process() -> None:
        ...
        if return_code not in (None, 0):
            messagebox.showerror(
                ...
                f"TokenShare Experiments CLI exited with code {return_code}.",
            )
    ...
    def start() -> None:
        try:
            ...
            messagebox.showerror("TokenShare Experiments", str(exc))
            return
        start_button.state(["disabled"])
        root.after(200, poll_process)
