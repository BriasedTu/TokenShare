"""TokenShare Experiments 对单个协议 root 的最小公共装配。"""
...
def _protocol_config(context: Any, system_dir: Path) -> ProtocolConfig:
    ...
    config = ProtocolConfig.default(
        config_id="experiments-" + "-".join(str(item) for item in _root_key(context)),
        artifact_store_uri=artifact_dir.resolve().as_uri(),
        event_log_uri=event_path.resolve().as_uri(),
        metadata={"experiments_profile_id": str(context.profile_id)},
    )
    return replace(
        config,
...
def _lean_environment() -> LeanEnvironmentManifest:
    ...
    project = _REPO_ROOT / "benchmarks" / "experiments" / "fixtures" / "lean_proof_project"
...
def _build_root_assembly(context: Any) -> RootAssembly:
    ...
    if experiment_id not in {"exp2", "exp3", "exp4"}:
        raise ValueError(f"unsupported experiments experiment: {experiment_id}")
    ...
    *,
    ...
    for planned in targets:
        ...
        attempt = SimpleNamespace(
            attempt_id=f"experiments_tail_plan:{identity}:0",
            client_id="experiments-coverage-tail",
            created_at=created_at,
            started_at=created_at,
            attempt_ordinal=0,
        )
        lease = SimpleNamespace(
            lease_id=f"experiments_tail_lease:{identity}:0",
            fencing_token=f"experiments-tail:{identity}:0",
...
def _lean_pre_dispatch_failure_trace(
    *,
    ...
    attempt = AttemptResultV1(
        attempt_id=f"experiments_tail_pre_dispatch:{identity}:0",
    ...
    if not targets:
        ...
        if pending:
            ...
            entry = ProviderEntryViewV1(
                ...
                endpoint="/not-used",
                api_key_env="EXPERIMENTS_RESUME_NOT_USED",
