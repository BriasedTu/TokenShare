...
def build_representative_unified_acquisition_plan(
    *,
    ...
    if not acquisition_roots:
        raise ValueError("representative acquisition selected no Exp1--4 roots")
    ...
    for root in acquisition_roots:
        ...
        prepared_values = freeze_formal_root_prepared_replacement_requests(
            root=root,
    ...
    for row in semantic_plan.rows:
        ...
        if (
            getattr(source_config, "config_digest", None)
        ...
        pricing_body = source_entries[0].pricing
        expected_pricing = FrozenPricing(
...
def _append_condition_ref(
    ...
    if candidate.formal_authority:
        ...
        required_strings = {
            ...
            if key not in {"seed", "split_profile_id"}
        }
        if (
