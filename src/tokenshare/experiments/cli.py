"""TokenShare Experiments 唯一命令行编排入口。"""
...
_DEFAULT_OUTPUT_ROOT = _REPO_ROOT / "TokenShareData" / "outputs" / "experiments"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tokenshare-experiments")
...
def _production_execute_root(context: _CliRootContext) -> RootResultV2:
    ...
    if not callable(execute):
        raise RuntimeError("TokenShare Experiments production root assembly is unavailable")
...
def _production_resume_root(
    ...
    if not callable(resume):
        raise RuntimeError("TokenShare Experiments protocol resume is unavailable")
...
def _provider_config_paths(
    ...
    if "exp1" in experiment_ids:
        paths.append(
            repo_root / "configs" / "experiments" / "exp1_baseline_provider_config.v3.json"
        )
...
def _case_inputs(repo_root: Path = _REPO_ROOT) -> dict[str, Mapping[str, Any]]:
    paths = (
        repo_root / "benchmarks" / "experiments" / "factorization_catalog.v2.jsonl",
...
def _run_lock_path(run_dir: Path) -> Path:
    return run_dir.parent / f".{run_dir.name}.experiments.lock"
...
def _journal_resume_entry(root: RootInventoryV1) -> ProviderEntryViewV1:
    ...
    return ProviderEntryViewV1(
        ...
        endpoint="/not-used",
        api_key_env="EXPERIMENTS_JOURNAL_RESUME_NOT_USED",
...
def _validate_resume_config(store: RunStore, config: ExperimentRunConfigV1) -> None:
    try:
        document = store.read_run_config()
        if set(document) != set(ExperimentRunConfigV1.field_names()):
            raise ValueError("run config fields differ from ExperimentRunConfigV1")
        frozen = ExperimentRunConfigV1(**document)
        frozen.validate()
