...
def formal_exp3_scheduled_target_unit_ids_by_case(
    *,
    condition: PaperExperimentCondition,
    ordered_case_ids: Sequence[str],
    planned_ai_unit_ids_by_case: Mapping[str, Sequence[str]],
) -> dict[str, tuple[str, ...]]:
    """按正式 Exp3 fault/death policy 投影每题会触达的 AI units。"""

    _validate_baseline_condition(condition)
    key = _parse_condition_id(condition.condition_id)
    expected = _condition_from_key(
        key,
        catalog_digest=condition.catalog_digest,
        endpoint_identity=_identity_from_condition(condition),
    )
    _validate_condition_matches_expected(condition, expected)
    case_ids = _case_tuple(ordered_case_ids)
    if set(planned_ai_unit_ids_by_case) != set(case_ids):
        raise ValueError("formal Exp3 target projection case coverage mismatch")
    units_by_case = {
        case_id: tuple(
            unit_id
            if unit_id.startswith(f"{case_id}:")
            else f"{case_id}:{unit_id}"
            for unit_id in _case_tuple(planned_ai_unit_ids_by_case[case_id])
        )
        for case_id in case_ids
    }
    if any(not units for units in units_by_case.values()):
        raise ValueError("formal Exp3 target projection requires planned AI units")
    if key.matrix_kind == "rate_fault":
        selected = set(
            select_fault_targets(
                (
                    unit_id
                    for case_id in case_ids
                    for unit_id in units_by_case[case_id]
                ),
                fault_rate=key.fault_rate_percent / 100.0,
                seed=RATE_FAULT_TARGET_SEED,
            )
        ...
        return {
            case_id: tuple(
                unit_id for unit_id in units_by_case[case_id] if unit_id in selected
            )
            for case_id in case_ids
        }
    if key.dead_worker_count is None or key.kill_progress_percent is None:
        raise ValueError("formal worker-death target policy is incomplete")
    return {
        case_id: _worker_death_targets(
            units_by_case[case_id],
            dead_worker_count=key.dead_worker_count,
            kill_progress_percent=key.kill_progress_percent,
        )
        for case_id in case_ids
