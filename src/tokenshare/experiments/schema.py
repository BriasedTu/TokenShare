"""TokenShare Experiments 普通数据对象与字段级验证。
...
from . import EXPERIMENT_SCHEMA_VERSION
...
class PrematureMergeObservationV2:
    """Experiments-local recovery-premerge probe 的实际结果；checker 未到达保持 null。"""
...
@dataclass(slots=True)
class ExperimentRunConfigV1(SchemaRecordV1):
    ...
    def validate(self) -> None:
        if self.schema_version != RUN_CONFIG_SCHEMA_VERSION:
            raise SchemaValidationError("invalid ExperimentRunConfigV1 schema_version")
        ...
        if self.pricing_versions != PRICING_VERSIONS:
            raise SchemaValidationError(
                "pricing_versions must equal frozen experiments mappings"
            )
