...
- Task 23 historical real fixture accepted，commit=`8c1d7ccc`，严格 12 文件（原计划10+监督者按严格必要性批准 `paper_direct_results.py` 与对应 test）。canonical RED=`1 collection error in 0.27s`，初始 GREEN=`8 passed in 4.37s`。
- Task 23 reviewer 初轮 BLOCK=`2 Critical/1 Important`；normal expanded-root 的 authoritative final 只能由 merge-unit canonical + `MERGE_RECORDED` 投影，central boundary 因此增加 exact additive branch，不改 `ProtocolEngine`。follow-up BLOCK=`1 Critical/1 Important`；16 个 manifest/merge commitment cases 中 12 个 RED，修复后 `16 passed in 5.77s`。
- Task 23 最终 central direct suite=`39 passed in 23.21s`、Task23 canonical=`8 passed in 6.55s`，reviewer PASS=`0/0/0`。direct result/table 只从持久化 ledger/artifact/manifest 重投影；Raw/Parsed/Candidate/merge wrapper 分离，完整 `ArtifactRef` 与 canonical/merge/terminal commitment fail closed，无 `last_verification`/自报 stages/影子 projector。
- Task 23 子智能体 post-accept test minimization=`CHANGED`，仅删重复断言/等价 case；两测试=`46 passed in 25.13s`。positive 24-file source tree与 2026-07-31 negative 2484-file tree均保持原 hash；tracked fixture最小脱敏，`regression_only`、`paper_eligible=false`，provider/network=`0/0`。
- Task 24 deterministic traceability replay accepted，commit=`8863e8b3`，严格 6 个计划文件。canonical RED=`1 collection error in 0.67s`，初始 GREEN=`137 passed in 126.11s`；reviewer 首轮 BLOCK=`1 Critical/2 Important`，persisted-root RED=`9 failed in 14.04s`→`17 passed in 36.67s`，canonical=`146 passed in 140.19s`。
- Task 24 follow-up BLOCK=`0 Critical/2 Important`；atomic promotion/contaminated-root RED=`2 failed,1 passed`→`3 passed`，最终 canonical=`149 passed in 151.15s`，reviewer PASS=`0/0/0`。L4 只接受 protected persisted descriptor，每次 fresh reload direct/current/source/current-provider；observations/tables/cell-lineage 独立 digest 一致，canonical order 与 numeric/null/online/trace roles闭合。
- Task 24 renderer+lineage+report 使用单一 owned-output generation 事务；replay descriptor 只追随 v2 manifest→condition CURRENT→generation→typed artifact-index closure，拒绝 metrics/paper/audit/CSV/TEX 等 derived 输入。provider/network/source writes/online fill=`0/0/0/0`。
- Task 24 子智能体 post-accept test minimization=`CHANGED`，合并两个重复测试、减少三次重复 L4 replay；traceability=`17 passed in 34.04s`，9 个计划测试和真实缺陷回归保留。
- Task 25 online-check plan/evidence contract accepted，commit=`c7db1948`，严格 4 个计划文件。RED semantic exit=`2`、`2 collection errors in 0.38s`，初始 GREEN=`28 passed in 12.65s`；reviewer 首轮 BLOCK=`1 Critical/2 Important`，官方 callback/typed provider objects/tracked authority 修复后 canonical=`28 passed in 28.34s`。
- Task 25 follow-up BLOCK=`1 Critical/1 Important`；capability 强制绑定 typed Factor verifier rejection 或真实 rejected Lean checker report、initial raw/candidate/attempt、official requeue 与 ordinal-1 new attempt；Exp3 false-positive 与 worker-death/kill-plan/ordinal identity 分离。最终 canonical=`29 passed in 37.64s`，reviewer PASS=`0/0/0`。
- Task 25 冻结 capability=`4 calls`、Exp2=`24 refs/480 upper`、Exp3=`2 roots/12 upper`、`max_concurrent_roots=1`；11 类 persisted objects 分离，usage/cost/wasted inputs 从 current payload 派生，缺 role blocked/null；无公式/post-bank/receipt/CLI。子智能体 post-min=`CHANGED`，最终=`29 passed in 36.34s`，provider/network=`0/0`。
...
- EPD-027 是实验设施全面改造，涉及稳定 provider-body digest、response-bank schema/inventory、trace-backed executor、source/consumer 双 provenance、双 paper evidence class、预算分账与硬门、runner/metrics/report/renderer/replay/audit；不是现有 replay 的局部修补。Task 0–20 已 accepted，但其余 `14/35` 仍未实施，正式全量继续 **NO-GO**。
- Experiment 5 指标继续遵守 EPD-025：维持零 retry，正文使用“首次输出质量/最终结果”和“调用量/资源”两张 model 汇总表，删除六组 pairwise、recovery 与重复旧正确率名。Task 2 metric contract、Task 3 direct projector、Task 4 stable request identity、Task 5 immutable bank primitives、Task 6 semantic inventory/preflight、Task 7 atomic budget authority、Task 8 durable acquisition、Task 9 logical scheduler、Task 10 parent-owned commit ABI、Task 11 trace-backed executor / dual provenance、Task 12–16 Exp1–5 projectors、Task 17 registry/formal metrics、Task 18 evidence eligibility、Task 19 normal formal lifecycle 与 Task 20 cell lineage observations 已 accepted；renderer/replay 仍属后续 Task，不能把 `21/35` 写成整条 pipeline implemented。
...
- EPD-025/027 的实施已不再是“只完成设计同步”：Task 2–20 的 metric contract、canonical direct results、stable request identity、immutable bank、inventory/preflight、budget、acquisition、logical scheduler、parent-owned commit ABI、trace-backed executor / dual provenance、standalone Exp1–5 projectors、registry/formal metric drafts、versioned evidence eligibility、normal formal lifecycle 与 per-cell lineage observations 已 accepted；report/renderer、execute/replay 与论文管线正式接线仍须按后续 Task 完成。本轮没有启动真实 API。
- 按用户要求，本次审计/修复没有新运行 Full、LeanAudit、force-all、真实 API smoke 或正式矩阵；未检查人为注入攻击。上面的真实调用来自用户此前启动的历史 smoke。
- 历史审计时当前进程的 `DEEPSEEK_API_KEY` 非空；这只证明当时环境曾配置，不构成当前付费授权，也不把 secret 写入仓库或输出。Task 0–20 已 accepted，Task 21 为 queued/next；旧 Exp3/4 与 Exp5 smoke 继续暂停。
