...
\begin{tikzpicture}[
    ...
    arrow/.style={-{Latex[length=2.2mm]}, line width=0.55pt, draw=blue!70!black},
    faintarrow/.style={-{Latex[length=1.7mm]}, line width=0.35pt, draw=blue!45!black},
    lane/.style={font=\scriptsize\bfseries, text=blue!70!black, anchor=east}
]

\node[lane] at (-8.32,1.15) {Protocol core};
\node[lane] at (-8.32,-0.20) {Task plugin};
\node[lane] at (-8.32,-1.48) {Accepted evidence};

\node[phase] (p1) at (-6.95,2.42) {\textbf{1. Registration}};
\node[phase] (p2) at (-4.62,2.42) {\textbf{2. Decomposition}};
\node[phase] (p3) at (-2.29,2.42) {\textbf{3. Execution}};
\node[phase] (p4) at (0.04,2.42) {\textbf{4. Parsing}};
\node[phase] (p5) at (2.37,2.42) {\textbf{5. Verification}};
\node[phase] (p6) at (4.70,2.42) {\textbf{6. Merge}};
\node[phase] (p7) at (7.03,2.42) {\textbf{7. Settlement}};
...
\node[core] (c1) at (-6.95,1.15) {Record task spec, contributor constraints, and descriptor digest};
\node[core] (c2) at (-4.62,1.15) {Record child units, dependency edges, and merge slots};
\node[core] (c3) at (-2.29,1.15) {Lease executable units and record contributor attempts};
\node[core] (c4) at (0.04,1.15) {Preserve raw submissions and parser outcomes};
\node[core] (c5) at (2.37,1.15) {Record verification before canonical binding};
\node[core] (c6) at (4.70,1.15) {Gate parent progress on required child evidence};
\node[core] (c7) at (7.03,1.15) {Record contribution and reward evidence};

\node[plugin] (g1) at (-6.95,-0.20) {Declare task schema, parser, verifier, and merge policy};
\node[plugin] (g2) at (-4.62,-0.20) {Select the task-family-specific split policy};
\node[plugin] (g3) at (-2.29,-0.20) {Provide executable instructions and output contract};
\node[plugin] (g4) at (0.04,-0.20) {Convert raw output into typed candidate artifacts};
\node[plugin] (g5) at (2.37,-0.20) {Run the task-specific checker or verifier};
\node[plugin] (g6) at (4.70,-0.20) {Define the composition rule for child artifacts};
\node[plugin] (g7) at (7.03,-0.20) {Expose task-specific contribution semantics};

\node[evidence] (e1) at (-6.95,-1.48) {Frozen descriptor\\and root task};
\node[evidence] (e2) at (-4.62,-1.48) {Coverage proof\\and slot map};
\node[evidence] (e3) at (-2.29,-1.48) {Traceable attempt\\and raw artifact};
\node[evidence] (e4) at (0.04,-1.48) {Parsed candidate\\or parse failure};
\node[evidence] (e5) at (2.37,-1.48) {Verification\\report};
\node[evidence] (e6) at (4.70,-1.48) {Merge record\\and parent candidate};
\node[evidence] (e7) at (7.03,-1.48) {Replayable\\settlement record};

\foreach \x/\y/\z in {p1/c1/g1,p2/c2/g2,p3/c3/g3,p4/c4/g4,p5/c5/g5,p6/c6/g6,p7/c7/g7} {
    \draw[faintarrow] (\x.south) -- (\y.north);
    \draw[faintarrow] (\y.south) -- (\z.north);
}

\foreach \x/\y in {g1/e1,g2/e2,g3/e3,g4/e4,g5/e5,g6/e6,g7/e7} {
    \draw[faintarrow] (\x.south) -- (\y.north);
}

\draw[arrow] (-6.95,-2.28) -- (7.03,-2.28);
\node[
    rectangle,
    rounded corners=2pt,
    draw=blue!70!black,
...
{append-only event ledger + content-addressed artifacts make the pathway replayable};

\end{tikzpicture}
}
}
\caption{\textcolor{blue}{Protocol operational pathway. Each phase separates reusable protocol responsibility from task-specific plugin responsibility, and every transition leaves accepted evidence that can be replayed.}}
\label{fig:protocol-operational-pathway}
\end{figure}
