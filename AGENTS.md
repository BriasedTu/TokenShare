...
- 实验设计必须优先遵守 `tokenshare_latest_real_plugin_experiment_design.md`：Experiment 1 真实 AI 跨领域可行性与难度、Experiment 2 真实模型 trace 驱动的 worker 扩展性及缩小题集六档在线检查、Experiment 3 真实模型 trace 故障/worker death 恢复及小型在线恢复检查、Experiment 4 真实模型 trace 协议消融、Experiment 5 预注册 model-endpoint comparison。正式 Lean catalog 固定为 3 个 paper difficulty × 3 个 topic family × 每格 15 道，共 135 道；不得缩成 10–20 道区间或用 shallow case 补格。Experiment 1–4 的所有真实 API acquisition/online 检查固定使用官方 DeepSeek `deepseek-v4-pro` / `deepseek_v4_pro_exp1_baseline`，thinking enabled、`reasoning_effort=high`、`timeout_seconds=600`、`max_tokens=300000`，tracked v3 配置为 `exp1_baseline_provider_config.v3.json`，不得自动换模型。Experiment 5 不再使用 strong/weak/mixed 标签；当前 cohort v3 固定比较 SiliconFlow `zai-org/GLM-5.2`、`Qwen/Qwen3-14B`、`MiniMaxAI/MiniMax-M2.5`、`Pro/deepseek-ai/DeepSeek-V3`，公共 `timeout_seconds=600`、`max_tokens=32768`、单次 provider attempt；GLM/Qwen/MiniMax 启用 thinking 且 `thinking_budget=32768`，DeepSeek-V3 nonthinking。v1/v2 只供历史 replay。Experiment 1/5 与 Experiment 2/3 的在线检查必须当次调用真实 AI API；Experiment 2–4 主矩阵只允许消费可追溯到批准真实 API acquisition 的不可变 response bank，并必须完整重跑协议状态机、明确标为 `real_model_trace_protocol_run`，不得冒充当次 `real_transport`。旧 deterministic/scripted suite、direct 500 benchmark、toy demo 或 `lean_stub` 只能作为回归、输入来源或成本校准。
