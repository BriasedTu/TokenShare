...
### 论文协作路由（Paper Collaboration Route）

`paper/` 是独立论文写作工作区。论文写作任务从 `paper/AGENTS.md` 进入；普通 Slim V2 开发任务不要把论文正文加入默认启动上下文。

论文侧和仓库侧统一使用两级交接：

1. `paper/PAPER_WRITING_BRIEF.md` 是唯一默认共同基线，保存论文需要理解的系统与实验主线、未实现边界和写作准则。根目录 Agent 只有在当前任务涉及论文交接，或实现／实验变化会使其中总体事实过期时才读取或更新它；普通开发启动不强制读取。
2. `paper/IMPLEMENTATION_DETAILS.md` 是可选的代码级参考。只有论文某个具体段落需要更细颗粒度，或当前实现变化命中其失效条件时，才定点读取对应部分。
3. `paper/DETAIL_REQUESTS.md` 是唯一按需提问通道。论文侧使用描述性标题提出一个具体问题；仓库侧在同一条目直接回答行为、代码或输出位置、验证、限制和核验日期，不建立编号式请求或证据数据库。
4. 若局部核验改变了系统或实验的总体理解，仓库侧同步更新 `PAPER_WRITING_BRIEF.md`；若只影响一个具体段落，答案保留在 `DETAIL_REQUESTS.md`，避免简报膨胀。
5. 仓库侧实现／实验 Agent 默认不得修改 `paper/paper.tex`、`paper/reference.bib` 或论文措辞；论文侧默认不得修改 `paper/` 外代码、配置或实验设施。
6. 细节请求只授权定点只读取证，不自动授权修改代码、调用真实 API、启动 Representative/Full 或其他有成本运行。当前代码和实际运行输出始终高于交接文档；相关事实变化后必须重新核验并将旧局部答案标记为 `stale`。
